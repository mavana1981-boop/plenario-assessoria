        ...
        if ordem_oficial:
            ...
            for chave_pdf in sorted(
                ...
                if chave_pdf not in reqsn_para_api:
                    # Fallback: pega qualquer REQ da API ainda não mapeado
                    for item in (itens_raw or []):
                        if not re.match(r'RE[QCS]', item['codigo'].upper()):
                            continue
                        if item not in reqsn_para_api.values():
                            reqsn_para_api[chave_pdf] = item
                            logger.info(f"REQSN fallback (qualquer REQ disponível): {chave_pdf} → {item['codigo']}")
                            break
                    if chave_pdf not in reqsn_para_api:
                        logger.warning(f"REQSN sem match e sem REQ disponível na API: {chave_pdf}")
