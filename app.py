...
<body>
        ...
        <div class="card-body">
          <div class="item-header" data-bs-toggle="collapse" data-bs-target="#col-{{ item.ordem }}-{{ item.id_principal }}" aria-expanded="false">
            <div>
              ...
              <span class="span-autor" id="autor-{{ item.id_principal }}" data-id="{{ item.id_principal }}">{{ item.autor }}</span>
  ...
  <script>
    ...
    // Carrega resumos de todos os itens sequencialmente (não bloqueia a UI)
...
(function() {
  ...
  function renderCard(item, container) {
    ...
    container.innerHTML = `
      <div style="background:#fff;border-radius:10px;padding:11px 12px 0;position:relative;
        ...
        ${imgSrc ? `
        <div style="width:100%;height:${imgH}px;border-radius:6px;overflow:hidden;margin:4px 0 6px;background:#fff;display:flex;align-items:center;justify-content:center;">
          <img src="${imgSrc}" style="max-width:100%;max-height:${imgH}px;object-fit:contain;">
        </div>` : `
        <div class="icon-img-placeholder" data-id="${id}" style="width:100%;height:${Math.round(imgH*0.5)}px;border-radius:6px;margin:4px 0 6px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:11px;">
          ${_imgBusy[id] ? '⏳ Buscando imagem…' : ''}
        </div>`}
  ...
  function buildPrintHTML(data){
    ...
    const card=it=>{
      ...
      const shadow=it.prioridade?`0 0 0 2.5px ${C.amber},0 6px 14px rgba(0,0,0,.10)`:'0 3px 10px rgba(0,0,0,.06)';
      const prio=it.marcador?`<div style="position:absolute;top:6px;right:6px;background:${C.amber};color:#fff;font-size:8px;font-weight:900;padding:2px 6px;border-radius:4px;">★ ${e(it.marcador)}</div>`:'';
      const img=it.imagem?`<div style="width:100%;height:${Math.round(IMG_H_BASE*(it.scale||1))}px;border-radius:6px;overflow:hidden;margin:4px 0 6px;background:#fff;display:flex;align-items:center;justify-content:center;"><img src="${it.imagem}" style="max-width:100%;max-height:${Math.round(IMG_H_BASE*(it.scale||1))}px;object-fit:contain;"></div>`:'';
      return`<div style="background:#fff;border-radius:10px;padding:11px 12px 0;position:relative;box-shadow:${shadow};overflow:hidden;page-break-inside:avoid;">
        <div style="position:absolute;left:0;top:0;bottom:0;width:4px;background:${cfg.bg};"></div>
        ${prio}
        <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:4px;padding-left:2px;">
          <div style="width:22px;height:22px;border-radius:50%;background:${C.inkSoft};color:#fff;font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${e(it.num)}</div>
          <div style="font-family:'Barlow Condensed',sans-serif;font-size:15px;font-weight:800;color:${C.ink};line-height:1.1;">${e(it.codigo)}</div>
        </div>
        ${img}
        <div style="font-size:9px;color:${C.inkSoft};line-height:1.5;padding-left:2px;">
        ...
        <div style="display:flex;align-items:flex-end;gap:8px;margin-top:6px;padding-left:2px;padding-bottom:8px;">
          <div style="flex:1;font-size:10px;color:#2c3e50;line-height:1.4;">${e(it.descricao)}</div>
          <div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0;">
            ${svgIco(cfg.icon,cfg.bg)}
            <div style="font-size:8px;font-weight:900;color:${cfg.bg};margin-top:2px;letter-spacing:.5px;">${e(it.orientacao||'—')}</div>
          </div>
        </div>
        <div style="height:4px;background:linear-gradient(90deg,${C.leaf} 0%,${C.forest} 100%);margin-left:-12px;margin-right:-12px;"></div>
      </div>`;
    };
    const logos=`<div style="display:flex;justify-content:center;align-items:center;gap:26px;margin-top:12px;">
      ${data.logo_min?`<img src="${data.logo_min}" style="height:38px;object-fit:contain;">`:`<div style="font-size:10px;font-weight:900;color:${C.forest};">LIDERANÇA DA MINORIA</div>`}
      ${data.logo_opo?`<img src="${data.logo_opo}" style="height:38px;object-fit:contain;">`:`<div style="font-size:10px;font-weight:900;color:${C.ruby};">OPOSIÇÃO</div>`}
    </div>`;
...
<style>@page{size:A4;margin:0}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}body{margin:0;padding:0;background:${C.cream};font-family:'Barlow',sans-serif;color:${C.ink}}.page{width:794px;min-height:1123px;margin:0 auto;background:${C.cream};padding-bottom:30px;position:relative}.pattern{position:absolute;inset:0;opacity:.04;pointer-events:none;background-image:radial-gradient(${C.ink} 1px,transparent 1px);background-size:16px 16px}.col{display:flex;flex-direction:column;gap:14px}</style></head><body>
<div class="page"><div class="pattern"></div>
<div style="padding:24px 28px 10px;text-align:center;">
  <div style="font-family:'Barlow Condensed',sans-serif;font-size:26px;font-weight:800;color:${C.ink};line-height:1.15;">
    Pauta Iconográfica — Plenário da Câmara<br/><span style="font-size:16px;font-weight:600;">${e(data.data)}</span>
  </div>${logos}
</div>
<div style="padding:12px 22px 0;display:grid;grid-template-columns:1fr 1fr;gap:14px;">
