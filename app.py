        ...
        <div class="card-body">
          <div class="item-header" data-bs-toggle="collapse" data-bs-target="#col-{{ item.ordem }}-{{ item.id_principal }}" aria-expanded="false">
            <div>
              ...
              <span class="span-autor" id="autor-{{ item.id_principal }}" data-id="{{ item.id_principal }}">{{ item.autor | truncar_autores }}</span>
...
<script>
  ...
  async function gerarQuadroDTQ(btn) {
    ...
    try {
        ...
        '<div class="quadro">' +
          '<div class="header">' +
            '<div class="header-titulo">' +
              '<div style="font-size:15px;font-weight:900;letter-spacing:.5px;">' + numero + (autoria ? ' — ' + autoria : '') + '</div>' +
              '<div style="font-size:12px;font-weight:400;margin-top:4px;opacity:.9;">' + descricao + '</div>' +
            '</div>' +
      ...
      // Botão editar quadro — torna campos contenteditable
      ...
      btnEdit.onclick = function() {
        pd.querySelectorAll('.header-titulo div,.voto-conteudo-sim,.voto-conteudo-nao,.explicacao-texto').forEach(el => {
          el.contentEditable = el.contentEditable === 'true' ? 'false' : 'true';
          el.style.outline = el.contentEditable === 'true' ? '2px dashed #f90' : '';
        });
        btnEdit.textContent = btnEdit.textContent.includes('Editar') ? '✅ Concluir edição' : '✏️ Editar Quadro';
      };
...
async function carregarOrientacoes() {
  ...
  try {
    ...
    j.forEach(o => {
      ...
      console.log('oriData key:', key, '→', o.orientacao);
    });
    console.log('oriData final:', oriData);
    renderizarTabela();
  } catch(e) {
...
function renderizarTabela() {
  ...
  ITENS_ORI.forEach((item) => {
    ...
    const resumo = (resumoEl && resumoEl.textContent.trim()) || (window.resumoCache && window.resumoCache[item.id_principal]) || '';
...
(function() {
  ...
  function renderCard(item, container) {
    ...
    container.innerHTML = `
      <div style="background:#fff;border-radius:10px;padding:11px 12px 0;position:relative;
        ...
        <!-- Controles inline (barra inferior) -->
        ...
        <div style="display:flex;justify-content:center;margin:6px 0 4px;">
          <div style="width:68px;height:68px;border-radius:50%;overflow:hidden;border:3px solid ${cfg.bg};box-shadow:0 2px 10px rgba(0,0,0,.18);flex-shrink:0;">
            <img src="${imgSrc}" style="width:100%;height:100%;object-fit:cover;display:block;">
          </div>
        </div>` : `
        <div class="icon-img-placeholder" data-id="${id}" style="width:36px;height:3px;background:${cfg.bg};border-radius:2px;margin:5px 0;">
          ${_imgBusy[id] ? '' : ''}
        </div>`}
  ...
  async function abrirModalIconografica() {
    ...
    // Busca imagens em background
    buscarTodasImagens(itens);
  }
  ...
  function buildPrintHTML(data){
    ...
    const left=data.items.slice(0,mid),right=data.items.slice(mid);
    const card=it=>{
      const cfg=oriCfg(it.orientacao);
      const shadow=it.marcador?'0 0 0 3px '+C.amber+',0 4px 12px rgba(0,0,0,.12)':'0 2px 8px rgba(0,0,0,.07)';
      const prio=it.marcador?'<div style="position:absolute;top:6px;right:6px;background:'+C.amber+';color:#fff;font-size:7.5px;font-weight:900;padding:2px 5px;border-radius:3px;letter-spacing:.5px;">★ '+e(it.marcador)+'</div>':'';
      const imgBlock = it.imagem
        ? '<div style="display:flex;justify-content:center;align-items:center;margin:6px 0 4px;"><div style="width:68px;height:68px;border-radius:50%;overflow:hidden;border:3px solid '+cfg.bg+';box-shadow:0 2px 10px rgba(0,0,0,.18);flex-shrink:0;"><img src="'+it.imagem+'" style="width:100%;height:100%;object-fit:cover;display:block;"></div></div>'
        : '<div style="width:36px;height:3px;background:'+cfg.bg+';border-radius:2px;margin:5px 0;"></div>';
      return`<div style="background:#fff;border-radius:10px;padding:10px 11px 0;position:relative;box-shadow:\${shadow};overflow:hidden;page-break-inside:avoid;border-top:3px solid \${cfg.bg};">
        \${prio}
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:2px;">
          <div style="width:20px;height:20px;border-radius:50%;background:\${C.inkSoft};color:#fff;font-size:10px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0;">\${e(it.num)}</div>
          <div style="font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:800;color:\${C.ink};line-height:1.1;flex:1;">\${e(it.codigo)}</div>
          \${svgIco(cfg.icon,cfg.bg)}
        </div>
        \${imgBlock}
        <div style="font-size:8.5px;color:\${C.inkSoft};line-height:1.5;">
          <div><strong>Autor:</strong> \${e(it.autor)}</div>
          <div><strong>Relator:</strong> \${e(it.relator)}</div>
        </div>
        <div style="font-size:9.5px;color:#2c3e50;line-height:1.4;margin-top:4px;padding-bottom:8px;">\${e(it.descricao)}</div>
        <div style="height:3px;background:\${cfg.bg};margin-left:-11px;margin-right:-11px;"></div>
        <div style="text-align:center;padding:2px 0 4px;font-size:8px;font-weight:900;color:\${cfg.bg};letter-spacing:.8px;">\${e(it.orientacao||'—')}</div>
      </div>`;
    };

    const logosMin = data.logo_min
      ? '<img src="'+data.logo_min+'" style="height:40px;object-fit:contain;padding-right:10px;border-right:1.5px solid #ccc;">'
    ...
    const logosOpo = data.logo_opo
      ? '<img src="'+data.logo_opo+'" style="height:40px;object-fit:contain;padding-left:10px;">'
      : '<span style="font-size:8px;font-weight:900;color:'+C.ruby+';padding-left:10px;">OPOSIÇÃO</span>';
    const logosHeader = '<div style="display:flex;align-items:center;justify-content:center;gap:0;margin-top:8px;border-top:1px solid #ddd;padding-top:8px;">'
      + logosMin + logosOpo + '</div>';
...
<style>@page{size:A4;margin:0}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}body{margin:0;padding:0;background:${C.cream};font-family:'Barlow',sans-serif;color:${C.ink}}.page{width:794px;min-height:1123px;margin:0 auto;background:${C.cream};padding-bottom:30px;position:relative}.pattern{position:absolute;inset:0;opacity:.04;pointer-events:none;background-image:radial-gradient(${C.ink} 1px,transparent 1px);background-size:16px 16px}.col{display:flex;flex-direction:column;gap:10px}</style></head><body>
<div class="page"><div class="pattern"></div>
<div style="padding:16px 24px 6px;border-bottom:2px solid ${C.ink}22;">
  <div style="text-align:center;font-family:'Barlow Condensed',sans-serif;font-size:22px;font-weight:800;color:${C.ink};">
    Pauta Iconográfica — Plenário da Câmara
    <span style="display:block;font-size:14px;font-weight:500;color:${C.inkSoft};">${e(data.data)}</span>
  </div>
  ${logosHeader}
</div>
<div style="padding:10px 18px 0;display:grid;grid-template-columns:1fr 1fr;gap:10px;position:relative;">
  ...
  <div class="col">${right.map(card).join('')}</div>
    ...
    w.document.write(buildPrintHTML({data:dataEvento, items:itens, logo_min, logo_opo}));
    w.document.close();
    w.focus();
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    document.getElementById('btn-iconografica')?.addEventListener('click', abrirModalIconografica);
    document.getElementById('btn-gerar-iconografica')?.addEventListener('click', gerarIconografica);
  });
})();
</script>
</body>
