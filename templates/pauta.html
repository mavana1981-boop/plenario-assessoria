...
<body>
  ...
  <div class="container my-4">
    ...
    <div class="d-flex align-items-center flex-wrap">
      ...
      <button class="btn btn-sm mb-3 ms-2 text-white" id="btn-pauta-banner"
        onclick="var d=document.getElementById('modalPautaBanner');var f=typeof window._abrirPautaBanner;if(f==='function'){window._abrirPautaBanner();}else{alert('DEBUG Pauta Banner:\nmodal='+(d?'OK':'AUSENTE')+'\n_abrirPautaBanner='+f);}"
      ...
      {% if user_categoria != 'restrito' %}
      <button class="btn btn-sm mb-3 ms-2 text-white"
        style="background:linear-gradient(135deg,#7B5E00,#B8920A);"
        onclick="abrirModalAdicionarProposicao()">
        <i class="fas fa-plus-circle me-2"></i>Extra Pauta
      </button>
      <button class="btn btn-sm mb-3 ms-2 text-white"
        style="background:linear-gradient(135deg,#1A3A5C,#0d2a4a);"
        onclick="abrirModalBuscarProposicao()">
        <i class="fas fa-search me-2"></i>Buscar Proposição
      </button>
      {% endif %}
      <button class="btn btn-sm mb-3 ms-2 text-white"
        style="background:linear-gradient(135deg,#B8860B,#DAA520);"
        onclick="var m=document.getElementById('modalOrientacoes');var f=typeof carregarOrientacoes;if(m&&f==='function'){carregarOrientacoes();new bootstrap.Modal(m).show();}else{alert('DEBUG Orientações:\nmodal='+(m?'OK':'AUSENTE')+'\ncarregarOrientacoes='+f);}"
        >
            ...
            {% else %}
              ...
              <div class="mt-4">
                <div class="d-flex align-items-center gap-2 mb-2 flex-wrap">
                  ...
                  <button class="btn btn-outline-primary btn-sm"
                          onclick="window.gerarTabelaDestaques('{{ item.projeto }}')">
                    <i class="fas fa-table me-1"></i>Tabela de Destaques
                  </button>
  ...
  <script>
    console.log('[PautaBanner] Template v3 carregado - 2026-06-26');
...
<script>
  ...
  async function buscarUrlPrlp(btn, idProp, numeroPrlp) {
    ...
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
    try {
      console.log('[DTQ] Buscando documentos para', idPrincipal);
      const r = await fetch('/listar_documentos/' + idPrincipal);
      if (!r.ok) throw new Error('HTTP ' + r.status);
      const j = await r.json();
      const docs = j.documentos || [];
      console.log('[DTQ] Documentos encontrados:', docs.length, docs.map(d=>d.label));
      if (docs.length === 0) {
  ...
  function gerarTabelaDestaques(projetoFiltro) {
    ...
    // Estrutura HTML real: o botão com data-numero fica numa <tr>, e o box de
    // análise (id="analise-destaque-...") fica na <tr> SEGUINTE, dentro de um
    // <td colspan>. Por isso usamos a <tr> ANTERIOR (previousElementSibling)
    // para localizar o botão de origem, em vez de closest('tr').
    const destaques = [];
    document.querySelectorAll('[id^="analise-destaque-"]').forEach(box => {
      if (box.style.display === 'none' || !box.innerHTML.trim()) return;

      const trAnalise = box.closest('tr');
      const trBotao    = trAnalise ? trAnalise.previousElementSibling : null;
      const btnOrigem  = trBotao ? trBotao.querySelector('[data-numero]') : null;
      ...
      // O texto da análise está sempre num <div class="analise-texto">,
      // criado separadamente do botão "Criar Quadro do DTQ" em todos os fluxos
      // de analisarDestaque/_executarAnaliseDestaque.
      const innerDiv = box.querySelector('.analise-texto') || box.querySelector('div');
      const analise  = (innerDiv ? innerDiv.innerText : box.innerText).trim();

      if (numero && analise) destaques.push({ numero, descricao, autoria, projeto, analise });
    });

    // Fallback: se nenhuma análise foi localizada via box, tenta pelos botões "Criar Quadro do DTQ",
    // usando o dataset.analise (texto da IA já salvo no próprio botão) sem cortar em 200 chars.
    if (destaques.length === 0) {
      document.querySelectorAll('[data-numero][onclick*="gerarQuadroDTQ"], .btn-dark[data-numero]').forEach(bq => {
        const analiseTxt = bq.dataset.analise ? bq.dataset.analise.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim() : '';
        if (!analiseTxt) return;
        destaques.push({
  ...
  async function gerarQuadroDTQ(btn) {
    ...
    // Abre a janela IMEDIATAMENTE (de forma síncrona, no gesto de clique do usuário)
    // para evitar bloqueio de popup pelo browser. Preenche o conteúdo depois.
    const popup = window.open('', '_blank', 'width=900,height=700,scrollbars=yes,resizable=yes');
    if (!popup) {
      alert('O navegador bloqueou a janela de pop-up. Permita pop-ups para este site e tente novamente.');
      btn.disabled = false;
      btn.innerHTML = '🖼️ Criar Quadro do DTQ';
      return;
    }
    const pd = popup.document;
    pd.open(); pd.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Quadro DTQ</title></head><body><p style="font-family:Arial,sans-serif;padding:30px;color:#666;">⏳ Gerando quadro...</p></body></html>'); pd.close();
    ...
    try {
      ...
      const j = await r.json();
      if (!j.ok) { alert('Erro: ' + j.error); popup.close(); return; }
      const d = j.dados;

      // Limpa o "carregando" e reconstrói o documento do popup já aberto
  ...
  async function _executarAnaliseDestaque(btn, idPrincipal, descricao, numero, projeto, extraParams) {
    ...
    if (ehEmenda) {
      ...
      try {
        ...
        if (j.resumo) {
          ...
          box.innerHTML = '';
          // Texto da análise SEMPRE num <div class="analise-texto"> próprio,
          // separado do botão "Criar Quadro", para gerarTabelaDestaques conseguir
          // localizar o texto de forma inequívoca.
          const divAnalise = document.createElement('div');
          divAnalise.className = 'analise-texto';
          divAnalise.innerHTML = j.resumo;
          box.appendChild(divAnalise);
          // Botão criar quadro
    ...
    try {
      ...
      console.log('[DTQ] Documentos encontrados:', docs.length, docs.map(d=>d.label));
      if (docs.length === 0) {
        // Para 'votação em separado', usa o texto da proposição diretamente
        const ehSeparado = /em separado/i.test(descricao);
        if (ehSeparado) {
          // Chama analisar_destaque sem documento específico
          btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
          const rS = await fetch('/analisar_destaque', {
            method: 'POST', headers: {'Content-Type': 'application/json'},
          ...
          if (jS.resumo) {
            const numKey = numero.replace(/\s/g, '-');
            let box = document.getElementById('analise-destaque-' + numKey);
            if (!box) { const tr=btn.closest('tr'); const ntr=tr?tr.nextElementSibling:null; if(ntr) box=ntr.querySelector('[id^="analise-destaque-"]'); }
            if (!box) { box=document.createElement('div'); box.className='mt-2 p-2 border rounded bg-light'; btn.closest('td')?.appendChild(box); }
            box.style.display='block'; box.innerHTML=jS.resumo;
          } else { alert('Erro: ' + (jS.error||'Falha')); }
          btn.disabled=false; btn.innerHTML='<i class="fas fa-robot"></i>';
          return;
        }
        alert('Nenhum documento encontrado para análise.'); return;
      }
      ...
      // Remove qualquer backdrop/modal ativo antes de abrir o novo
      ...
      btnDebug.addEventListener('click', async () => {
        ...
        try {
          ...
          bSalvar.addEventListener('click', () => {
            ...
            inner.innerHTML = '';
            const dRes = window.document.createElement('div'); dRes.className = 'analise-texto'; dRes.innerHTML = analiseHtml; inner.appendChild(dRes);
      ...
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-robot"></i>';
      alert('Erro ao analisar destaque: ' + e.message);
    }
  } // fim _executarAnaliseDestaque

  // Expõe funções no escopo global para uso em onclick dinâmico
  window.analisarDestaque        = analisarDestaque;
  window._executarAnaliseDestaque = _executarAnaliseDestaque;
  window.gerarQuadroDTQ          = gerarQuadroDTQ;
  window.verTextoDoc             = verTextoDoc;
  window.gerarTabelaDestaques    = gerarTabelaDestaques;
...
<div class="modal fade" id="modalPautaBanner" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-scrollable" style="max-width:860px;">
    <div class="modal-content">
      <div class="modal-header" style="background:linear-gradient(135deg,#1A6B3A,#0d4a27);color:#fff;">
        <h5 class="modal-title"><i class="fas fa-newspaper me-2"></i>Pauta Banner</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label class="form-label fw-bold small"><i class="fas fa-image me-1"></i>Imagem de fundo (opcional)</label>
          <input type="file" class="form-control form-control-sm" id="pb-imagem" accept="image/jpeg,image/png,image/webp">
        </div>
        <div id="pb-img-preview" style="display:none;margin-bottom:10px;">
          <img id="pb-img-preview-src" style="max-height:80px;border-radius:4px;border:1px solid #ddd;">
        </div>
        <div class="mb-2">
          <label class="form-label fw-bold small mb-1">
            <i class="fas fa-star me-1" style="color:#DAA520;"></i>Selecione os projetos em destaque/prioridade
          </label>
          <div class="form-text mb-2">Marque os itens que aparecerão no card ⭐ Prioridade.</div>
          <div id="pb-lista-prioridade"
            style="max-height:200px;overflow-y:auto;border:1px solid #dee2e6;border-radius:6px;padding:8px;">
            <div class="text-muted small fst-italic">Abrindo pauta...</div>
          </div>
        </div>
        <div id="pb-resultado" style="display:none;">
          <div style="display:flex;align-items:center;gap:8px;background:#FFF9E6;border:1px solid #F0C030;border-radius:6px;padding:8px 12px;margin-bottom:10px;">
            <i class="fas fa-pencil-alt" style="color:#B8860B;"></i>
            <span style="font-size:0.82rem;color:#7A5C00;"><strong>Modo edição:</strong> clique em qualquer campo para editar antes de baixar.</span>
          </div>
          <div id="pb-preview-outer" style="border:1px solid #ddd;border-radius:4px;overflow:auto;background:#C5D0DC;max-height:65vh;">
            <div id="pb-preview-inner" style="transform-origin:top left;width:794px;"></div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Fechar</button>
        <button type="button" class="btn btn-sm text-white" id="btn-pb-gerar"
          style="background:linear-gradient(135deg,#1A6B3A,#0d4a27);" onclick="gerarPautaBanner()">
          <i class="fas fa-magic me-1"></i>Gerar Banner
        </button>
        <button type="button" class="btn btn-sm text-white" id="btn-pb-gerar-lg"
          style="background:linear-gradient(135deg,#1A3A7A,#0d2a5c);" onclick="gerarPautaBannerLetraGrande()">
          <i class="fas fa-text-height me-1"></i>Letra Grande
        </button>
        <button type="button" class="btn btn-outline-primary btn-sm" id="btn-pb-png"
          style="display:none;" onclick="baixarPautaBannerPNG(this)">
...
document.addEventListener('DOMContentLoaded',function(){
  document.getElementById('pb-imagem')?.addEventListener('change',function(){
    const f=this.files[0],p=document.getElementById('pb-img-preview'),i=document.getElementById('pb-img-preview-src');
    if(f){const r=new FileReader();r.onload=e=>{i.src=e.target.result;p.style.display='block';};r.readAsDataURL(f);}
    else p.style.display='none';
  });
});
window._abrirPautaBanner=function(){
  document.getElementById('pb-resultado').style.display='none';
  document.getElementById('btn-pb-png').style.display='none';
  document.getElementById('pb-imagem').value='';
  document.getElementById('pb-img-preview').style.display='none';
  // Popula lista de prioridade com itens da pauta
  const lista=document.getElementById('pb-lista-prioridade');
  const itens=(window.ITENS_DATA||[]).filter(it=>{
    const p=it.projeto_original||it.projeto||'';
    const naoEhReq=/^REQ\s|^RQS\s|^RQU\s|^REC\s/i.test(p);
    const naoEhRedacao=/Reda[cç][aã]o Final/i.test(it.projeto);
    return !naoEhReq && !naoEhRedacao;
  });
  if(itens.length===0){
    lista.innerHTML='<div class="text-muted small fst-italic">Nenhum item disponível.</div>';
  } else {
    lista.innerHTML=itens.map(it=>{
      const temOrientacaoSim=(it.orientacao||'').toUpperCase()==='SIM';
      // Por padrão nenhum item vem marcado como prioridade no Pauta Banner
      return `<div class="form-check py-1" style="border-bottom:1px solid #f0f0f0;">`
        +`<input class="form-check-input" type="checkbox" id="pb-pri-${it.id_principal}"
           value="${it.id_principal}">`
        +`<label class="form-check-label small" for="pb-pri-${it.id_principal}">`
        +`<strong>Item ${it.ordem}</strong> — ${it.projeto}`
        +`${temOrientacaoSim?' <span class="badge" style="background:#DAA520;font-size:0.65rem;">SIM</span>':''}`
        +`</label></div>`;
    }).join('');
  }
  new bootstrap.Modal(document.getElementById('modalPautaBanner')).show();
};
function _ePB(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
async function gerarPautaBanner(){
  const btn=document.getElementById('btn-pb-gerar');
  btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-1"></i>Gerando...';
  try{
    const itens=window.ITENS_DATA||[];
    const imgFile=document.getElementById('pb-imagem').files[0];
    let imgB64='',imgMime='';
    if(imgFile){
      imgB64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(',')[1]);r.onerror=rej;r.readAsDataURL(imgFile);});
      imgMime=imgFile.type;
    }
    const [logoOpoB64,logoMinB64]=await Promise.all([
      fetch('/static/logo_oposicao.png').then(r=>r.blob()).then(b=>new Promise(res=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.readAsDataURL(b)})),
      fetch('/static/logo_minoria.png').then(r=>r.blob()).then(b=>new Promise(res=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.readAsDataURL(b)})),
    ]).catch(()=>['','']);
    const dataEl=document.querySelector('[data-evento-data]');
    const dataEvt=dataEl?.dataset?.eventoData||'';
    let dataFmt='';
    if(dataEvt){
      // data-evento-data vem como DD/MM/YYYY do datetimeformat
      const meses=['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro'];
      const partes=dataEvt.split('/');
      if(partes.length===3){
        const dia=parseInt(partes[0]),mes=parseInt(partes[1]),ano=partes[2];
        if(!isNaN(dia)&&!isNaN(mes)) dataFmt=`${dia} de ${meses[mes-1]} de ${ano}`;
      }
    }
    const urgencias=itens.filter(it=>/^REQ\s|^RQS\s|^RQU\s|^REC\s/i.test((it.projeto_original||it.projeto||'')));
    const materias=itens.filter(it=>/Reda[cç][aã]o Final/i.test(it.projeto)&&!urgencias.includes(it));
    // Prioridade: exclusivamente os checkboxes marcados pelo usuário (nenhuma marcação por padrão)
    const checksSel=Array.from(document.querySelectorAll('#pb-lista-prioridade input[type=checkbox]:checked')).map(cb=>String(cb.value));
    const prioridade=itens.filter(it=>checksSel.includes(String(it.id_principal))&&!urgencias.includes(it)&&!materias.includes(it));
    const projetos=itens.filter(it=>!urgencias.includes(it)&&!materias.includes(it)&&!prioridade.includes(it));
    const html=_buildPBHv4({materias,urgencias,projetos,prioridade,dataFmt,imgB64,imgMime,logoOpoB64,logoMinB64});
    const inner=document.getElementById('pb-preview-inner');
    inner.innerHTML=html;
    const outerW=document.getElementById('pb-preview-outer').offsetWidth||700;
    const scale=Math.min(1,(outerW-8)/794);
    inner.style.transform=`scale(${scale})`;inner.style.width='794px';inner.style.transformOrigin='top left';
    setTimeout(()=>{const h=inner.scrollHeight*scale;document.getElementById('pb-preview-outer').style.height=Math.min(h+20,window.innerHeight*0.6)+'px';},100);
    _ativarEdicaoPautaBanner(inner);
    document.getElementById('pb-resultado').style.display='block';
    document.getElementById('btn-pb-png').style.display='inline-block';
  }catch(e){alert('Erro: '+e.message);}
  finally{btn.disabled=false;btn.innerHTML='<i class="fas fa-magic me-1"></i>Gerar Banner';}
}

// ── PAUTA BANNER LETRA GRANDE ────────────────────────────────────────────────
// Gera múltiplas páginas A4 (794px) com fonte mínima 16px
// Separa: urgências / matérias / projetos — se muitos itens, cada seção em página própria
async function gerarPautaBannerLetraGrande(){
  const btn = document.getElementById('btn-pb-gerar-lg');
  btn.disabled = true;
  ...
  try{
    ...
    if(dataEvt){
      const meses=['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro'];
      const p=dataEvt.split('/');
      if(p.length===3){const d=parseInt(p[0]),m=parseInt(p[1]),a=p[2];if(!isNaN(d)&&!isNaN(m))dataFmt=`${d} de ${meses[m-1]} de ${a}`;}
    }
    ...
    const materias  = itens.filter(it=>/Reda[cç][aã]o Final/i.test(it.projeto)&&!urgencias.includes(it));
    const checksSel = Array.from(document.querySelectorAll('#pb-lista-prioridade input[type=checkbox]:checked')).map(cb=>String(cb.value));
    const priIds    = new Set(checksSel);
    const todosProj = itens.filter(it=>!urgencias.includes(it)&&!materias.includes(it)).sort((a,b)=>(a.ordem||0)-(b.ordem||0));

    const oriCores = {'SIM':'#1A6B3A','NÃO':'#B91C1C','NAO':'#B91C1C','LIBERADO':'#2A5C8A','OBSTRUÇÃO':'#7A1A1A','NEGOCIAÇÃO':'#8A6A00','ABSTENÇÃO':'#555'};
    const logoHtml = [logoOpoB64,logoMinB64].filter(Boolean).map(l=>`<img src="${l}" style="height:38px;object-fit:contain;">`).join('');
    const cabFoto  = imgB64 ? `<img src="data:${imgMime};base64,${imgB64}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;"><div style="position:absolute;inset:0;background:rgba(10,20,10,0.6);z-index:1;"></div>` : '';

    // ── HTML helpers ────────────────────────────────────────────────────
    function mkCabecalho(){
      return `<div style="background:#0d2a0d;padding:16px 20px;position:relative;overflow:hidden;">
        ${cabFoto}
        <div style="position:relative;z-index:2;display:flex;justify-content:space-between;align-items:center;">
          <div>
            <div style="font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:900;color:#fff;line-height:.9;letter-spacing:-1px;">PAUTA<br><span style="color:#E9B847">RESUMIDA</span></div>
            <div style="font-size:12px;font-weight:700;color:rgba(255,255,255,.7);letter-spacing:2px;text-transform:uppercase;margin-top:4px;">Ordem do Dia — Câmara dos Deputados</div>
            <div style="margin-top:6px;background:#1A6B3A;display:inline-flex;align-items:center;gap:6px;padding:3px 10px;border-radius:4px;">
              <span style="color:#fff;font-size:14px;font-weight:700;">${dataFmt||'—'}</span>
            </div>
          </div>
          <div style="background:#fff;border-radius:6px;padding:6px 10px;display:flex;gap:8px;align-items:center;">${logoHtml}</div>
        </div>
      </div><div style="height:6px;background:#E9B847;"></div>`;
    }

    function mkRodape(){
      return `<div style="background:#1A2A3A;padding:8px 16px;display:flex;justify-content:space-between;align-items:center;">
        <div style="display:flex;gap:8px;align-items:center;">${logoHtml}</div>
        <div style="text-align:right;font-size:10px;color:rgba(255,255,255,.7);">
          <div style="font-weight:800;letter-spacing:1px;text-transform:uppercase;">Pauta Resumida — Ordem do Dia</div>
          <div>${dataFmt||''}</div>
        </div>
      </div>`;
    }

    function mkSecaoHdr(ico, titulo, cor, cont){
      return `<div style="background:${cor};padding:10px 14px;display:flex;align-items:center;gap:10px;">
        <span style="font-size:18px;">${ico}</span>
        <span style="font-family:'Barlow Condensed',sans-serif;font-size:20px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#fff;">${titulo}${cont?' <span style="font-size:14px;font-weight:600;opacity:.8">(Continuação)</span>':''}</span>
      </div>`;
    }

    function mkItem(it, tipo){
      const ori = (it.orientacao||'').toUpperCase();
      const oriBadge = ori?`<span style="background:${oriCores[ori]||'#555'};color:#fff;font-size:11px;font-weight:900;padding:3px 10px;border-radius:4px;margin-left:8px;">${ori}</span>`:'';
      const proj  = _e(it.projeto||'');
      const autor = _e((it.autor||'').split(',').slice(0,2).join(', ').substring(0,70));
      const rel   = tipo!=='urg'?_e((it.relator||'').replace('Não atribuído','').substring(0,50)):'';
      const res   = _e((window.resumoCache&&window.resumoCache[String(it.id_principal)])||it.ementa||'');
      const n     = String(it.ordem);
      const ehPri = priIds.has(String(it.id_principal))&&tipo!=='urg';
      const bgNum = tipo==='urg'?'#8B3A3A':(ehPri?'#7A5500':'#2A4A8A');
      const bgRow = ehPri?'background:#FFFBEA;border-left:4px solid #E9B847;':'background:#fff;';
      return `<div style="padding:10px 14px;border-bottom:1px solid #eee;${bgRow}display:flex;gap:10px;align-items:flex-start;">
        <div style="min-width:30px;height:30px;background:${bgNum};color:#fff;border-radius:4px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:14px;flex-shrink:0;">${ehPri?'⭐':n}</div>
        <div>
          <div style="font-size:18px;font-weight:800;color:${tipo==='urg'?'#5A1A1A':(ehPri?'#7A5500':'#1A2A5C')};">${proj}${oriBadge}</div>
          ${autor?`<div style="font-size:16px;color:#444;margin-top:2px;">Autor: ${autor}</div>`:''}
          ${rel?`<div style="font-size:16px;color:#666;">Rel.: ${rel}</div>`:''}
          ${res?`<div style="font-size:16px;color:#333;margin-top:3px;line-height:1.4;">${res.substring(0,140)}</div>`:''}
        </div>
      </div>`;
    }

    // ── Mede alturas reais via DOM ───────────────────────────────────────
    btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Medindo layout...';
    const med = document.createElement('div');
    med.style.cssText = 'position:fixed;left:-9999px;top:0;width:794px;visibility:hidden;z-index:-1;';
    document.body.appendChild(med);

    async function medir(html){
      med.innerHTML = html; void med.offsetHeight;
      await new Promise(r=>requestAnimationFrame(r));
      return med.firstElementChild ? med.firstElementChild.offsetHeight : 0;
    }

    const hCab = await medir(mkCabecalho());
    const hRod = await medir(mkRodape());
    const hHdr = await medir(mkSecaoHdr('⚡','TESTE','#8B3A3A',false));

    // Mede cada item
    const grupos = [
      {ico:'⚡',titulo:'Requerimentos de Urgência',cor:'#8B3A3A',itens:urgencias,tipo:'urg'},
      {ico:'📄',titulo:'Matérias sobre a Mesa',    cor:'#2E7D52', itens:materias, tipo:'mat'},
      {ico:'🏛️',titulo:'Projetos em Discussão',    cor:'#2A4A8A',itens:todosProj,tipo:'proj'},
    ].filter(g=>g.itens.length>0);

    for(const g of grupos){
      g.alturas = [];
      for(const it of g.itens){
        g.alturas.push(await medir(mkItem(it,g.tipo))+1);
      }
    }
    document.body.removeChild(med);

    // ── Distribui em páginas sem jamais cortar um item ───────────────────
    const AREA = 1123 - hCab - hRod - 20; // área útil real com 10px margem cada lado
    const paginas = []; // cada página = array de {tipo:'hdr'|'item', html, h}

    let paginaAtual = [], altAtual = 0;

    function novasPagina(){
      if(paginaAtual.length>0) paginas.push(paginaAtual);
      paginaAtual = []; altAtual = 0;
    }

    for(const g of grupos){
      let cont = false;
      for(let i=0; i<g.itens.length; i++){
        const hItem = g.alturas[i];
        const hdr   = mkSecaoHdr(g.ico, g.titulo, g.cor, cont);
        // Se é primeiro item do grupo OU continuação: precisa do header
        const precisaHdr = (i===0 || cont===false);
        // Calcula se header+item cabem
        const espacoNecessario = (i===0||cont)?hHdr+hItem:hItem;
        if(altAtual + espacoNecessario > AREA){
          novasPagina();
          cont = true;
        }
        // Adiciona header se necessário
        if(i===0 || (cont && paginaAtual.length===0)){
          const hdrHtml = mkSecaoHdr(g.ico, g.titulo, g.cor, cont&&i>0);
          paginaAtual.push({html:`<div style="border-radius:8px 8px 0 0;overflow:hidden;margin-bottom:0;">${hdrHtml}</div>`,h:hHdr});
          altAtual += hHdr;
        }
        // Adiciona item
        paginaAtual.push({html:mkItem(g.itens[i],g.tipo), h:hItem});
        altAtual += hItem;
      }
      // Fecha o card da seção (borda inferior)
      if(paginaAtual.length>0){
        paginaAtual.push({html:'<div style="height:12px;background:#fff;border-bottom:1.5px solid #ddd;border-radius:0 0 8px 8px;margin-bottom:16px;"></div>',h:12+16});
        altAtual += 12+16;
      }
    }
    novasPagina();

    // ── Renderiza cada página e captura ─────────────────────────────────
    ...
    for(let p=0; p<paginas.length; p++){
      btn.innerHTML = `<i class="fas fa-spinner fa-spin me-1"></i>Capturando ${p+1}/${paginas.length}...`;
      const corpo = paginas[p].map(x=>x.html).join('');
      const pageHTML = `<div style="width:794px;background:#F5F5F3;font-family:'Source Sans 3',Arial,sans-serif;">
        ${mkCabecalho()}
        <div style="padding:10px 12px;">${corpo}</div>
        ${mkRodape()}
      </div>`;
      ...
      // Captura com altura exata da página
      const canvasPage = await html2canvas(el,{
        scale:3, useCORS:true, allowTaint:true,
        backgroundColor:'#F5F5F3', logging:false,
        width:794, height:el.scrollHeight,
        windowWidth:794, windowHeight:el.scrollHeight,
      });

      // Cria canvas A4 exato e desenha a página nele (sem escalar — tamanho certo)
      const A4c = document.createElement('canvas');
      A4c.width = 794*3; A4c.height = 1123*3;
      const ctx = A4c.getContext('2d');
      ctx.fillStyle = '#F5F5F3';
      ctx.fillRect(0,0,A4c.width,A4c.height);
      ctx.drawImage(canvasPage,0,0);
      dataURLs.push(A4c.toDataURL('image/png',1.0));
    }
    document.body.removeChild(container);

    // ── Abre popup ───────────────────────────────────────────────────────
    const popup = window.open('','_blank','width=880,height=900,scrollbars=yes,resizable=yes');
    if(!popup){alert('Permita pop-ups.');return;}
    const imgsHTML = dataURLs.map((src,i)=>
      `<div class="pagina"><img src="${src}"><div class="num">Pág. ${i+1}/${dataURLs.length}</div></div>`
    ).join('');
    const pd = popup.document;
    pd.open();
    pd.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pauta — Letra Grande</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  body{background:#555;font-family:Arial,sans-serif;}
  .toolbar{background:#1a1a1a;color:#fff;padding:10px 18px;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:99;}
  .toolbar strong{flex:1;font-size:13px;}
  .toolbar button{padding:7px 18px;border:none;border-radius:5px;cursor:pointer;font-weight:700;font-size:13px;}
  .btn-print{background:#1A6B3A;color:#fff;} .btn-close{background:#555;color:#fff;}
  .paginas{padding:16px;display:flex;flex-direction:column;align-items:center;gap:12px;}
  .pagina{position:relative;}
  .pagina img{width:794px;display:block;box-shadow:0 4px 20px rgba(0,0,0,.5);}
  .num{position:absolute;bottom:6px;right:12px;font-size:11px;color:rgba(255,255,255,.5);}
  @media print{
    body{background:#fff;} .toolbar{display:none!important;}
    .paginas{padding:0;gap:0;}
    .pagina{page-break-after:always;break-after:page;}
    .pagina img{width:100%;} .num{display:none;}
    @page{size:A4 portrait;margin:0;}
  }
</style></head><body>
<div class="toolbar">
  <strong>📋 Pauta Resumida — Letra Grande · ${dataURLs.length} página${dataURLs.length>1?'s':''}</strong>
  <button class="btn-print" onclick="window.print()">🖨️ Imprimir / PDF</button>
  <button class="btn-close" onclick="window.close()">✕ Fechar</button>
</div>
<div class="paginas">${imgsHTML}</div>
</body></html>`);
    pd.close();
    ...
    btn.disabled=false;
    btn.innerHTML='<i class="fas fa-text-height me-1"></i>Letra Grande';
  }
}

function _ativarEdicaoPautaBanner(c){c.querySelectorAll('[data-pb-edit]').forEach(el=>{el.contentEditable='true';el.style.outline='none';el.style.cursor='text';el.addEventListener('focus',function(){this.style.boxShadow='0 0 0 2px rgba(255,200,0,0.7)';this.style.borderRadius='2px';});el.addEventListener('blur',function(){this.style.boxShadow='';});});}
function _buildPBHv4({materias,urgencias,projetos,prioridade,dataFmt,imgB64,imgMime,logoOpoB64,logoMinB64}){
  const C={v:'#1A6B3A',ve:'#0d4a27',a:'#E9B847',c:'#2C3E50',f:'#F4F6F8'};

  const cabBgColor = 'background:#0d2a0d;';
  const cabFotoEl = imgB64
    ? `<img src="data:${imgMime};base64,${imgB64}"
        style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;object-position:center top;display:block;z-index:0;">
       <div style="position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(to right,rgba(0,0,0,0.72) 0%,rgba(0,0,0,0.20) 40%,rgba(0,0,0,0.20) 60%,rgba(0,0,0,0.72) 100%);z-index:1;"></div>`
    : '';

  const logoBox = (logoOpoB64||logoMinB64) ? `<div style="background:#fff;border-radius:5px;padding:6px 10px;display:flex;align-items:center;gap:8px;flex-shrink:0;">
    ${logoOpoB64?`<img src="${logoOpoB64}" style="height:38px;width:auto;object-fit:contain;">`:''}
    ${logoMinB64?`<img src="${logoMinB64}" style="height:38px;width:auto;object-fit:contain;">`:''}
  </div>` : '';

  function ePB(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}

  const priIds = new Set(prioridade.map(it=>String(it.id_principal)));
  const todosProj = [...prioridade,...projetos].sort((a,b)=>(a.ordem||0)-(b.ordem||0));

  // ── Renderização dos itens ────────────────────────────────────────────────

  function badgeOrientacao(it){
    const ori=(it.orientacao||'').toUpperCase().trim();
    if(!ori) return '';
    const cores={
      'SIM':         {bg:'#1A6B3A',fg:'#fff'},
      'NÃO':         {bg:'#B91C1C',fg:'#fff'},
      'NAO':         {bg:'#B91C1C',fg:'#fff'},
      'LIBERADO':    {bg:'#5B6470',fg:'#fff'},
      'OBSTRUÇÃO':   {bg:'#7A1A1A',fg:'#fff'},
      'OBSTRUCAO':   {bg:'#7A1A1A',fg:'#fff'},
      'ABSTENÇÃO':   {bg:'#8A6D00',fg:'#fff'},
      'ABSTENCAO':   {bg:'#8A6D00',fg:'#fff'},
      'NEGOCIAÇÃO':  {bg:'#1E5C8A',fg:'#fff'},
      'NEGOCIACAO':  {bg:'#1E5C8A',fg:'#fff'},
    };
    const c=cores[ori]||{bg:'#6c757d',fg:'#fff'};
    return`<span style="position:absolute;top:6px;right:8px;background:${c.bg};color:${c.fg};font-size:8px;font-weight:800;letter-spacing:0.5px;text-transform:uppercase;padding:2px 6px;border-radius:3px;white-space:nowrap;z-index:2;">${ePB(ori)}</span>`;
  }

  function itemUrgencia(it){
    const n=String(it.ordem),p=ePB(it.projeto||'');
    const a=ePB((it.autor||'').split(',').slice(0,2).join(', ').substring(0,60));
    const res=ePB((window.resumoCache&&window.resumoCache[String(it.id_principal)])||it.ementa||'');
    return`<div style="position:relative;padding:8px 28px 8px 12px;border-bottom:1px solid #f0f0f0;display:flex;gap:7px;align-items:flex-start;font-size:10.5px;color:#222;">
      ${badgeOrientacao(it)}
      <div style="min-width:20px;height:20px;background:#7A1A1A;color:#fff;border-radius:3px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:9.5px;flex-shrink:0;">${n}</div>
      <div data-pb-edit><b>${p}</b>${a?` <span style="color:#666;">(${a})</span>`:''}${res?`<div style="color:#444;margin-top:1px;font-size:10px;line-height:1.35;">${res.substring(0,120)}</div>`:''}</div>
    </div>`;
  }

  function itemMateria(it){
    const n=String(it.ordem),p=ePB(it.projeto||'');
    const a=ePB((it.autor||'').split(',').slice(0,2).join(', ').substring(0,60));
    const res=ePB((window.resumoCache&&window.resumoCache[String(it.id_principal)])||it.ementa||'');
    return`<div style="position:relative;padding:10px 80px 10px 12px;border-bottom:1px solid #f0f0f0;display:flex;gap:9px;align-items:flex-start;">
      ${badgeOrientacao(it)}
      <div style="min-width:26px;height:26px;background:#1A3A2A;color:#fff;border-radius:4px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:11px;flex-shrink:0;">${n}</div>
      <div data-pb-edit style="font-size:11px;color:#1a1a1a;">
        <div style="font-weight:800;color:#1A3A2A;">${p}</div>
        ${a?`<div style="font-size:10px;color:#555;">Autor: ${a}</div>`:''}
        ${res?`<div style="margin-top:2px;color:#333;line-height:1.4;">${res.substring(0,160)}</div>`:''}
      </div>
    </div>`;
  }

  function itemProjeto(it, compacto){
    const n=String(it.ordem),p=ePB(it.projeto||'');
    const a=ePB((it.autor||'').split(',').slice(0,2).join(', ').substring(0,55));
    const rel=ePB((it.relator||'').replace('Não atribuído','').substring(0,40));
    const res=ePB((window.resumoCache&&window.resumoCache[String(it.id_principal)])||it.ementa||'');
    const ehPri=priIds.has(String(it.id_principal));
    const maxRes = compacto ? 100 : 160;
    const badgeN = ehPri
      ? `<div style="display:flex;align-items:center;gap:2px;flex-shrink:0;">`
        +`<div style="height:${compacto?20:26}px;background:#7A5500;color:#fff;border-radius:${compacto?3:4}px;padding:0 4px;display:flex;align-items:center;justify-content:center;font-size:${compacto?8:10}px;white-space:nowrap;">⭐ ${n}</div>`
        +`</div>`
      : `<div style="min-width:${compacto?20:26}px;height:${compacto?20:26}px;background:#1A2A5C;color:#fff;border-radius:${compacto?3:4}px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:${compacto?9:11}px;flex-shrink:0;">${n}</div>`;
    const rowBg=ehPri?'background:#FFFBEA;border-left:3px solid #E9B847;':'';
    const padV=compacto?'7px':'10px';
    const padL=compacto?'10px':'12px';
    const gap=compacto?'7px':'9px';
    const padRight = compacto ? 70 : 90;
    return`<div style="position:relative;padding:${padV} ${padRight}px ${padV} ${padL};border-bottom:1px solid #f0f0f0;${rowBg}display:flex;gap:${gap};align-items:flex-start;">
      ${badgeOrientacao(it)}
      ${badgeN}
      <div data-pb-edit style="font-size:${compacto?10:11}px;color:#1a1a1a;">
        <div style="font-weight:800;color:${ehPri?'#7A5500':'#1A2A5C'};">${p}</div>
        ${a?`<div style="font-size:${compacto?9:10}px;color:#555;">Autor: ${a}</div>`:''}
        ${(!compacto&&rel)?`<div style="font-size:10px;color:#777;">Rel.: ${rel}</div>`:''}
        ${res?`<div style="margin-top:2px;color:#333;line-height:1.4;">${res.substring(0,maxRes)}</div>`:''}
      </div>
    </div>`;
  }

  function cardHeader(ico,titulo,cor){
    return`<div style="background:${cor};padding:9px 13px;display:flex;align-items:center;gap:9px;">
      <div style="width:28px;height:28px;background:rgba(255,255,255,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">${ico}</div>
      <span style="font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#fff;" data-pb-edit>${titulo}</span>
    </div>`;
  }

  function wrapCard(conteudo){
    return`<div style="background:#fff;border-radius:8px;overflow:hidden;border:1px solid #c8d8c8;margin-bottom:10px;">${conteudo}</div>`;
  }
  function wrapCardCor(cor){
    return function(conteudo){
      return`<div style="background:#fff;border-radius:8px;overflow:hidden;border:1px solid ${cor};margin-bottom:10px;">${conteudo}</div>`;
    };
  }

  // ── Decisão de layout ─────────────────────────────────────────────────────
  // Conta itens em cada "lado"
  const nEsq = (materias||[]).length + (urgencias||[]).length;
  const nDir = todosProj.length;

  // LAYOUT A — 2 colunas simétricas (equilibrado)
  // LAYOUT B — vertical: urgências/matérias no topo (linha inteira),
  //            projetos embaixo em 2 sub-colunas side-by-side
  // Usa B quando projetos > 2× (matérias+urgências)
  const usarLayoutVertical = nDir > 2 * Math.max(nEsq, 1);

  const cardMaterias  = (materias&&materias.length)
    ? wrapCard(cardHeader('📄','Matérias sobre a Mesa','#1A3A2A') + materias.map(itemMateria).join('')) : '';
  const cardUrgencias = (urgencias&&urgencias.length)
    ? wrapCardCor('#c8a8a8')(cardHeader('⚡','Requerimentos de Urgência','#7A1A1A') + urgencias.map(itemUrgencia).join('')) : '';

  let corpo;

  if(!usarLayoutVertical){
    // LAYOUT A: 2 colunas — esquerda matérias+urgências, direita projetos
    const cardProjetos = todosProj.length
      ? wrapCardCor('#a8b8d8')(cardHeader('🏛️','Projetos em Discussão','#1A2A5C') + todosProj.map(it=>itemProjeto(it,false)).join('')) : '';
    corpo = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:10px 10px 0;align-items:start;">
      <div style="min-width:0;">${cardMaterias}${cardUrgencias}</div>
      <div style="min-width:0;">${cardProjetos}</div>
    </div>`;

  } else {
    // LAYOUT B: linha superior com matérias+urgências, linha inferior com projetos em 2 sub-colunas
    // Divide projetos em 2 sub-colunas com greedy por altura
    const H = 80;
    const sub1=[], sub2=[];
    let h1=0, h2=0;
    todosProj.forEach(it=>{
      if(h1<=h2){sub1.push(it);h1+=H;}
      else       {sub2.push(it);h2+=H;}
    });

    // Card de projetos com 2 sub-colunas internas
    const projBody = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:0;align-items:start;">
      <div style="border-right:1px solid #e8e8e8;">${sub1.map(it=>itemProjeto(it,true)).join('')}</div>
      <div>${sub2.map(it=>itemProjeto(it,true)).join('')}</div>
    </div>`;
    const cardProjetosLargo = todosProj.length
      ? wrapCardCor('#a8b8d8')(cardHeader('🏛️','Projetos em Discussão','#1A2A5C') + projBody) : '';

    // Urgências/matérias em largura total acima dos projetos
    const topoCards = [cardMaterias, cardUrgencias].filter(Boolean).join('');
    const topoBloco = topoCards
      ? `<div style="padding:10px 10px 0;">${topoCards}</div>` : '';

    corpo = topoBloco
      + `<div style="padding:${topoBloco?'0':'10px'} 10px 0;">${cardProjetosLargo}</div>`;
  }

  const rodape=`<div style="background:${C.c};padding:10px 14px;display:flex;align-items:center;justify-content:space-between;gap:10px;">
    <div style="display:flex;align-items:center;gap:8px;">
      ${logoOpoB64?`<img src="${logoOpoB64}" style="height:26px;object-fit:contain;">`:''}
      ${logoMinB64?`<img src="${logoMinB64}" style="height:26px;object-fit:contain;">`:''}
    </div>
    <div style="text-align:right;">
      <div style="font-size:9px;font-weight:800;color:#fff;letter-spacing:1px;text-transform:uppercase;" data-pb-edit>Pauta Resumida — Ordem do Dia</div>
      <div style="font-size:7.5px;color:rgba(255,255,255,.55);" data-pb-edit>Material de Apoio Parlamentar · ${dataFmt||new Date().toLocaleDateString('pt-BR')}</div>
    </div>
  </div>`;

  return`<link href="https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@700;800;900&family=Source+Sans+3:wght@400;600;700&display=swap" rel="stylesheet">
<div class="pb-banner" style="width:794px;background:${C.f};font-family:'Source Sans 3',Arial,sans-serif;border-radius:8px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.2);">
  <div style="${cabBgColor}padding:22px 18px 0;position:relative;overflow:hidden;">
    <div style="position:absolute;top:0;left:0;right:0;height:3px;background:${C.a};z-index:3;"></div>
    ${cabFotoEl}
    <div style="position:relative;z-index:2;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
        <div>
          <div style="font-family:'Barlow Condensed',sans-serif;font-size:56px;font-weight:900;color:#fff;line-height:.85;letter-spacing:-1px;" data-pb-edit>PAUTA<br><span style="color:${C.a}">RESUMIDA</span></div>
          <div style="font-family:'Barlow Condensed',sans-serif;font-size:12px;font-weight:700;color:rgba(255,255,255,.80);letter-spacing:2.5px;text-transform:uppercase;margin-top:7px;" data-pb-edit>Ordem do Dia — Câmara dos Deputados</div>
        </div>
        ${logoBox}
      </div>
      <table style="border-collapse:collapse;margin-top:12px;background:${C.v};border-radius:5px;"><tr>
        <td style="padding:5px 4px 5px 12px;vertical-align:middle;line-height:0;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></td>
        <td style="padding:5px 12px 5px 4px;vertical-align:middle;color:#fff;font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:700;white-space:nowrap;" data-pb-edit>${dataFmt||'—'}</td>
      </tr></table>
      <div style="height:14px;background:${C.f};margin:12px -18px 0;border-radius:12px 12px 0 0;"></div>
    </div>
  </div>
  ${corpo}
  <div style="padding:2px 10px 10px;"></div>
  ${rodape}
</div>`;
}

async function baixarPautaBannerPNG(btn){
  const orig=btn.innerHTML;btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-1"></i>Gerando PNG...';
  try{
    const el=document.querySelector('#pb-preview-inner .pb-banner');
    if(!el){alert('Gere o banner primeiro.');return;}
    const inner=document.getElementById('pb-preview-inner');
    const sc=inner.style.transform;
    inner.style.transform='none';
    // Força reflow e espera o browser repintar o layout sem o scale
    void el.offsetHeight;
    await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
    await new Promise(r=>setTimeout(r,150));
    const canvas=await html2canvas(el,{scale:3,useCORS:true,allowTaint:true,backgroundColor:'#ffffff',logging:false,width:el.scrollWidth,height:el.scrollHeight,windowWidth:el.scrollWidth,windowHeight:el.scrollHeight});
    inner.style.transform=sc;
    // PNG A4 pronto para impressao: 794px * 3 = 2382px de largura (suficiente para impressao sem corte)
    const a=document.createElement('a');a.download='pauta_banner_A4.png';a.href=canvas.toDataURL('image/png',1.0);document.body.appendChild(a);a.click();document.body.removeChild(a);
  }catch(e){alert('Erro: '+e.message);}finally{btn.disabled=false;btn.innerHTML=orig;}
}

// ── BANNER DE PROPOSIÇÃO — renderização na div ───────────────────────────────
function _ativarEdicaoBanner(c){
  const sels=['.cab-titulo','.cab-subtitulo','.cab-desc','.dado-valor','.dado-label','.dado-detalhe','.mc-val','.resumo','.preve-item span','.critica-titulo','.critica-detalhe','.just-txt','.ori-badge-txt','.arg-body-txt','.pratica-item span','.rodape-txt'];
  sels.forEach(s=>{(c||document).querySelectorAll(s).forEach(el=>{el.contentEditable='true';el.style.outline='none';el.style.cursor='text';el.addEventListener('focus',function(){this.style.boxShadow='0 0 0 2px rgba(255,200,0,0.6)';this.style.borderRadius='2px';});el.addEventListener('blur',function(){this.style.boxShadow='';});});});
  const av=document.getElementById('banner-edit-aviso');if(av)av.style.display='flex';
}
async function exportarBannerPNG(btn){
  console.log('[BannerPNG] clique recebido. _bannerHtmlAtual existe?', !!window._bannerHtmlAtual);
  const orig=btn?btn.innerHTML:'';
  if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-1"></i>Gerando PNG...';}
  try{
    const inner=document.getElementById('banner-preview-inner');
    console.log('[BannerPNG] inner encontrado?', !!inner, 'innerHTML length:', inner?inner.innerHTML.length:0);
    const el=inner ? inner.querySelector('.banner') : null;
    console.log('[BannerPNG] .banner encontrado dentro de inner?', !!el);
    if(!el){
      alert('Banner não encontrado no preview. Gere o banner primeiro e aguarde ele aparecer na tela antes de clicar em Baixar PNG.');
      return;
    }
    const sc=inner.style.transform;
    inner.style.transform='none';
    ...
    await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
    await new Promise(r=>setTimeout(r,150));
    console.log('[BannerPNG] dimensões do elemento:', el.scrollWidth, 'x', el.scrollHeight);
    const canvas=await html2canvas(el,{scale:2,useCORS:true,allowTaint:true,backgroundColor:'#ffffff',logging:true,width:el.scrollWidth,height:el.scrollHeight,windowWidth:el.scrollWidth,windowHeight:el.scrollHeight});
    inner.style.transform=sc;
    console.log('[BannerPNG] canvas gerado:', canvas.width, 'x', canvas.height);
    const a=document.createElement('a');
    a.download='banner_plenario.png';
    a.href=canvas.toDataURL('image/png',1.0);
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    console.log('[BannerPNG] download disparado');
  }catch(e){
    console.error('[BannerPNG] ERRO:', e);
...
// ── BANNER PROPOSIÇÃO — LETRA GRANDE ─────────────────────────────────────
async function exportarBannerLetraGrande(btn){
  const inner = document.getElementById('banner-preview-inner');
  const el    = inner ? inner.querySelector('.banner') : null;
  if(!el){ alert('Gere o banner primeiro.'); return; }
  ...
  try{
    // Pega orientação do modal
    const oriVal  = (document.getElementById('banner-orientacao')?.value || '').trim().toUpperCase();
    const oriCores = {'SIM':'#1A6B3A','NÃO':'#B91C1C','NAO':'#B91C1C','LIBERADO':'#1A3A7A',
                      'OBSTRUÇÃO':'#7A1A1A','OBSTRUCAO':'#7A1A1A','NEGOCIAÇÃO':'#8A6A00',
                      'NEGOCIACAO':'#8A6A00','ABSTENÇÃO':'#444','ABSTENCAO':'#444'};
    const oriCor   = oriCores[oriVal] || '#1A2A3A';

    // Aumenta temporariamente a fonte de todos os textos para 18px mínimo
    const todosEls = el.querySelectorAll('*');
    const fonteOriginal = [];
    todosEls.forEach((e,i) => {
      const fs = parseFloat(window.getComputedStyle(e).fontSize);
      fonteOriginal.push(e.style.fontSize);
      if(fs < 18) e.style.fontSize = '18px';
    });
    ...
    // Captura com scale 3 (alta resolução A4)
    const sc = inner.style.transform;
    inner.style.transform = 'none';
    void el.offsetHeight;
    await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));

    const canvasBanner = await html2canvas(el, {
      scale: 3,
      useCORS: true,
      allowTaint: true,
    ...
    // Restaura fontes originais
    todosEls.forEach((e,i) => { e.style.fontSize = fonteOriginal[i]; });
    inner.style.transform = sc;

    // Adiciona faixa de orientação no topo
    let canvas = canvasBanner;
    if(oriVal){
      const FAIXA_H = 90; // altura da faixa em px no canvas (scale 3)
      const finalC = document.createElement('canvas');
      finalC.width  = canvasBanner.width;
      finalC.height = canvasBanner.height + FAIXA_H;
      const ctx = finalC.getContext('2d');
      // Faixa
      ctx.fillStyle = oriCor;
      ctx.fillRect(0, 0, finalC.width, FAIXA_H);
      // Texto da orientação centralizado
      ctx.fillStyle = '#ffffff';
      ctx.font = `900 ${FAIXA_H * 0.55}px 'Barlow Condensed', Arial`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.letterSpacing = '4px';
      ctx.fillText('ORIENTAÇÃO: ' + oriVal, finalC.width / 2, FAIXA_H / 2);
      // Banner abaixo
      ctx.drawImage(canvasBanner, 0, FAIXA_H);
      canvas = finalC;
    }

    // Abre popup com a imagem para impressão A4
    const popup = window.open('', '_blank', 'width=880,height=700,scrollbars=yes,resizable=yes');
    if(!popup){ alert('Permita pop-ups.'); return; }
    const dataURL = canvas.toDataURL('image/png', 1.0);
    const pd = popup.document;
    pd.open();
    pd.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Banner — Letra Grande</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  body{background:#555;font-family:Arial,sans-serif;}
  .toolbar{background:#1a1a1a;color:#fff;padding:10px 18px;display:flex;align-items:center;gap:10px;position:sticky;top:0;z-index:99;}
  .toolbar strong{flex:1;font-size:13px;}
  .toolbar button{padding:7px 18px;border:none;border-radius:5px;cursor:pointer;font-weight:700;font-size:13px;}
  .btn-print{background:#1A6B3A;color:#fff;}
  .btn-dl{background:#1A3A7A;color:#fff;}
  .btn-close{background:#555;color:#fff;}
  .pagina{display:flex;justify-content:center;padding:16px;}
  img{max-width:794px;display:block;box-shadow:0 4px 20px rgba(0,0,0,.5);}
  @media print{
    body{background:#fff;}
...
<div class="toolbar">
  <strong>🖼️ Banner Proposição — Letra Grande</strong>
  <button class="btn-print" onclick="window.print()">🖨️ Imprimir / PDF</button>
  <button class="btn-dl" onclick="const a=document.createElement('a');a.download='banner_letra_grande.png';a.href=document.getElementById('img').src;document.body.appendChild(a);a.click();document.body.removeChild(a);">⬇️ Baixar PNG</button>
  <button class="btn-close" onclick="window.close()">✕ Fechar</button>
</div>
<div class="pagina"><img id="img" src="${dataURL}"></div>
</body></html>`);
    // Download direto via URL já disponível
    popup._dataURL = dataURL;
    pd.close();
...
// Listener para o botão Letra Grande do banner individual
document.addEventListener('click', function(ev){
  const btn = ev.target.closest('#btn-banner-letra-grande');
  if(btn) exportarBannerLetraGrande(btn);
});

// Listener robusto via delegação de evento — funciona mesmo se o botão for recriado/clonado
document.addEventListener('click', function(ev){
  const btnPng = ev.target.closest('#btn-exportar-banner-png');
  if(btnPng){
    console.log('[BannerPNG] clique detectado via delegação');
    exportarBannerPNG(btnPng);
  }
});
</script>

<div class="modal fade" id="modalGerarBanner" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-scrollable" style="max-width:900px;">
    <div class="modal-content">
      <div class="modal-header" style="background:linear-gradient(135deg,#1a3a5c,#0d2137);">
        <h5 class="modal-title text-white">
          <i class="fas fa-image me-2"></i>Gerar Banner &mdash; <span id="banner-modal-titulo"></span>
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        ...
        <div class="row mb-3">
          <div class="col-md-4">
            <label class="form-label fw-bold small">Orientação da Minoria</label>
            <select class="form-select form-select-sm" id="banner-orientacao">
          ...
          <div class="col-md-4">
            ...
            <input type="text" class="form-control form-control-sm" id="banner-regime" placeholder="Ex: Urgência">
          </div>
          <div class="col-md-4">
            <label class="form-label fw-bold small">Comissões</label>
            <input type="text" class="form-control form-control-sm" id="banner-comissoes" value="Plenário">
          </div>
        </div>

        <div class="mb-3">
          <label class="form-label fw-bold small">
            <i class="fas fa-image me-1"></i>Imagem de fundo do cabeçalho (opcional)
          </label>
          <input type="file" class="form-control form-control-sm" id="banner-imagem"
            accept="image/jpeg,image/png,image/webp">
          <div class="form-text">Será usada como fundo do cabeçalho, como no modelo de banner.</div>
        </div>

        <div id="banner-img-preview" class="mb-3" style="display:none;">
          <img id="banner-img-preview-src"
            style="max-height:110px;border-radius:6px;border:1px solid #ddd;display:block;">
        </div>

        <div class="mb-3">
          <label class="form-label fw-bold small">Contexto adicional para a IA (opcional)</label>
          <div id="banner-aviso-nota" class="alert alert-warning py-2 px-3 mb-2" style="font-size:0.8rem;display:none;">
            <i class="fas fa-exclamation-triangle me-1"></i>
            <strong>Nota técnica vazia.</strong> Abra o item e preencha a nota antes de gerar o banner — a IA usa exclusivamente esse texto.
          </div>
          <textarea class="form-control form-control-sm" id="banner-resumo-extra" rows="2"
            placeholder="Comentários adicionais para orientar a IA..."></textarea>
        </div>

        <div id="banner-resultado" style="display:none;">
          <hr>
          <div class="d-flex align-items-center justify-content-between mb-2 flex-wrap gap-2">
            <label class="fw-bold small mb-0">
              <i class="fas fa-check-circle text-success me-1"></i>Banner gerado
            </label>
            <button class="btn btn-outline-primary btn-sm" id="btn-exportar-banner-png" type="button">
              <i class="fas fa-image me-1"></i>Baixar PNG
            </button>
            <button class="btn btn-sm text-white" id="btn-banner-letra-grande" type="button"
              style="background:linear-gradient(135deg,#1A3A7A,#0d2a5c);">
              <i class="fas fa-text-height me-1"></i>Letra Grande
            </button>
          </div>
          <div id="banner-edit-aviso" style="display:none;align-items:center;gap:8px;
            background:#FFF9E6;border:1px solid #F0C030;border-radius:6px;padding:8px 12px;margin-bottom:8px;">
            <i class="fas fa-pencil-alt" style="color:#B8860B;"></i>
            <span style="font-size:0.82rem;color:#7A5C00;">
              <strong>Modo edição:</strong> clique em qualquer campo para editar antes de baixar.
            </span>
          </div>
          <div id="banner-preview-outer"
            style="border:1px solid #ddd;border-radius:4px;overflow:auto;background:#C5D0DC;">
            <div id="banner-preview-inner" style="transform-origin:top left;display:block;"></div>
          </div>
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Fechar</button>
        <button type="button" class="btn btn-primary btn-sm" id="btn-banner-gerar"
          onclick="gerarBannerIA()">
          <img src="/static/logo_gemini.svg" alt="Gemini"
            style="width:15px;height:15px;vertical-align:middle;" class="me-1">
          Gerar Banner com Gemini
        </button>
      </div>
    </div>
  </div>
...
let _bannerItemData = {};
let _bannerHtmlAtual = '';

function abrirModalBanner(btn) {
  const ordem = btn.dataset.ordem || '';
  // Pega nota técnica do Quill (texto puro, sem HTML)
  const quill = quillInstances[ordem];
  const notaTecnica = quill ? quill.getText().trim() : '';

  _bannerItemData = {
    proposicao:   btn.dataset.proposicao  || '',
    ementa:       btn.dataset.ementa      || '',
    autor:        btn.dataset.autor       || '',
    relator:      btn.dataset.relator     || '',
    idPrincipal:  btn.dataset.idPrincipal || '',
    ordem:        ordem,
    notaTecnica:  notaTecnica,
  };

  document.getElementById('banner-modal-titulo').textContent = _bannerItemData.proposicao;
  document.getElementById('banner-regime').value             = '';
  document.getElementById('banner-comissoes').value          = 'Plenário';
  document.getElementById('banner-resumo-extra').value       = '';
  document.getElementById('banner-imagem').value             = '';
  document.getElementById('banner-img-preview').style.display = 'none';
  document.getElementById('banner-resultado').style.display  = 'none';
  _bannerHtmlAtual = '';

  // Sincroniza orientação com o select da pauta se já estiver preenchido
  const oriEl = document.querySelector('.orientacao[data-ordem="' + _bannerItemData.ordem + '"]');
  if (oriEl && oriEl.value) {
    document.getElementById('banner-orientacao').value = oriEl.value;
  }

  // Avisa se não há nota técnica
  const aviso = document.getElementById('banner-aviso-nota');
  if (aviso) aviso.style.display = notaTecnica ? 'none' : 'block';

  new bootstrap.Modal(document.getElementById('modalGerarBanner')).show();
}

// Preview da imagem no modal
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('banner-imagem')?.addEventListener('change', function () {
    const file = this.files[0];
    const prev = document.getElementById('banner-img-preview');
    const img  = document.getElementById('banner-img-preview-src');
    if (file) {
      const reader = new FileReader();
      reader.onload = ev => { img.src = ev.target.result; prev.style.display = 'block'; };
...
async function gerarBannerIA() {
  const btn = document.getElementById('btn-banner-gerar');
  btn.disabled = true;
  btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Gerando com Gemini...';
  document.getElementById('banner-resultado').style.display = 'none';

  try {
    const formData = new FormData();
    formData.append('proposicao',    _bannerItemData.proposicao);
    formData.append('ementa',        _bannerItemData.ementa);
    formData.append('autor',         _bannerItemData.autor);
    formData.append('relator',       _bannerItemData.relator);
    formData.append('regime',        document.getElementById('banner-regime').value);
    formData.append('comissoes',     document.getElementById('banner-comissoes').value);
    formData.append('orientacao',    document.getElementById('banner-orientacao').value);
    formData.append('nota_tecnica',  _bannerItemData.notaTecnica || '');
    formData.append('resumo_extra',  document.getElementById('banner-resumo-extra').value);

    const imgFile = document.getElementById('banner-imagem').files[0];
    if (imgFile) formData.append('imagem', imgFile);

    const r = await fetch('/gerar_banner_proposicao', { method: 'POST', body: formData });
    const j = await r.json();

    if (!j.success) {
      alert('Erro ao gerar banner:\n' + (j.error || 'Falha desconhecida'));
      return;
    }

    _bannerHtmlAtual = j.html;

    // Usa o fragmento limpo (style + div.banner, sem doctype/head/scripts) que o backend
    // já monta separadamente — mesmo padrão usado no Pauta Banner, que funciona corretamente
    // com html2canvas.
    const htmlUtil = j.banner_html_fragmento || j.html;

    // Renderiza na div (sem iframe — evita cross-origin)
    const inner = document.getElementById('banner-preview-inner');
    inner.innerHTML = htmlUtil;
    const outerW = document.getElementById('banner-preview-outer').offsetWidth || 700;
    const scale  = Math.min(1, (outerW - 8) / 850);
    inner.style.transform = 'scale(' + scale + ')';
    inner.style.width = '850px';
    inner.style.transformOrigin = 'top left';
    setTimeout(() => {
      const h = inner.scrollHeight * scale;
      document.getElementById('banner-preview-outer').style.height = h + 'px'; // altura real do banner
    }, 100);
    _ativarEdicaoBanner(inner);
    document.getElementById('banner-resultado').style.display = 'block';

  } catch (err) {
    alert('Erro de conexão: ' + err.message);
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<img src="/static/logo_gemini.svg" alt="Gemini" style="width:15px;height:15px;vertical-align:middle;" class="me-1"> Gerar Banner com Gemini';
  }
}

function imprimirBanner() {
  if (!_bannerHtmlAtual) return;
  const blob = new Blob([_bannerHtmlAtual], { type: 'text/html;charset=utf-8' });
  const url  = URL.createObjectURL(blob);
  const popup = window.open(url, '_blank', 'width=900,height=750,scrollbars=yes,resizable=yes');
  if (!popup) {
...
<!-- ═══ MODAL ADICIONAR PROPOSIÇÃO EXTRA PAUTA ═══════════════════════════ -->
<div class="modal fade" id="modalAdicionarProposicao" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header" style="background:linear-gradient(135deg,#7B5E00,#B8920A);color:#fff;">
        <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i>Adicionar Proposição Extra Pauta</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <p class="text-muted small mb-3">Digite a sigla, número e ano da proposição. O sistema buscará os dados na API da Câmara.</p>
        <div class="row g-2">
          <div class="col-4">
            <label class="form-label small fw-bold">Sigla</label>
            <select class="form-select form-select-sm" id="ep-sigla">
              <option>PL</option><option>PLP</option><option>PEC</option>
              <option>MPV</option><option>PDL</option><option>PRC</option>
              <option>REQ</option><option>PLS</option>
            </select>
          </div>
          <div class="col-4">
            <label class="form-label small fw-bold">Número</label>
            <input type="number" class="form-control form-control-sm" id="ep-numero" placeholder="Ex: 1234">
          </div>
          <div class="col-4">
            <label class="form-label small fw-bold">Ano</label>
            <input type="number" class="form-control form-control-sm" id="ep-ano" placeholder="Ex: 2025">
          </div>
        </div>
        <div id="ep-resultado" class="mt-3" style="display:none;">
          <div class="alert alert-warning py-2 small mb-0">
            <i class="fas fa-star me-1" style="color:#B8860B;"></i>
            <strong>Extra Pauta</strong>
            <div id="ep-preview" class="mt-1"></div>
          </div>
        </div>
        <div id="ep-erro" class="mt-2 text-danger small" style="display:none;"></div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancelar</button>
        <button class="btn btn-warning btn-sm" id="ep-btn-buscar" onclick="buscarProposicaoExtra()">
          <i class="fas fa-search me-1"></i>Buscar
        </button>
        <button class="btn btn-sm text-white" id="ep-btn-adicionar" style="background:#7B5E00;display:none;" onclick="adicionarProposicaoExtra()">
          <i class="fas fa-plus me-1"></i>Adicionar à Pauta
        </button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="modalBuscarProposicao" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-scrollable" style="max-width:900px;">
    <div class="modal-content">
      <div class="modal-header" style="background:linear-gradient(135deg,#1A3A5C,#0d2a4a);color:#fff;">
        <h5 class="modal-title"><i class="fas fa-search me-2"></i>Buscar Proposição</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="row g-2 mb-3">
          <div class="col-3">
            <select class="form-select form-select-sm" id="bp-sigla">
              <option>PL</option><option>PLP</option><option>PEC</option>
              <option>MPV</option><option>PDL</option><option>PRC</option>
              <option>REQ</option>
            </select>
          ...
          <div class="col-3">
            <input type="number" class="form-control form-control-sm" id="bp-numero" placeholder="Número">
          </div>
          <div class="col-3">
            <input type="number" class="form-control form-control-sm" id="bp-ano" placeholder="Ano">
          </div>
          <div class="col-3">
            <button class="btn btn-primary btn-sm w-100" onclick="buscarProposicaoBP()">
              <i class="fas fa-search me-1"></i>Buscar
            </button>
          </div>
        </div>
        <div id="bp-loading" style="display:none;" class="text-center text-muted py-3">
          <i class="fas fa-spinner fa-spin me-2"></i>Buscando proposição e nota técnica...
        </div>
        <div id="bp-resultado" style="display:none;">
          <div class="card mb-3">
            <div class="card-header py-2" style="background:#1A3A5C;color:#fff;">
              <strong id="bp-titulo"></strong>
            </div>
            <div class="card-body py-2 small">
              <div><strong>Ementa:</strong> <span id="bp-ementa"></span></div>
              <div class="mt-1"><strong>Autor:</strong> <span id="bp-autor"></span> &nbsp; <strong>Relator:</strong> <span id="bp-relator"></span></div>
              <div class="mt-1"><strong>Situação:</strong> <span id="bp-situacao"></span></div>
            </div>
          </div>
          <div id="bp-nota-container">
            <div class="d-flex align-items-center justify-content-between mb-2">
              <strong class="small">Nota Técnica:</strong>
              <div class="d-flex gap-2">
                <button class="btn btn-sm btn-outline-primary" onclick="gerarNotaBP()">
                  <img src="/static/logo_gemini.svg" style="width:14px;height:14px;vertical-align:middle;" class="me-1">Gerar com IA
                </button>
                <button class="btn btn-sm btn-outline-success" onclick="imprimirNotaBP()">
                  <i class="fas fa-print me-1"></i>Imprimir PDF
                </button>
                <button class="btn btn-sm btn-outline-secondary" onclick="gerarBannerBP()">
                  <i class="fas fa-image me-1"></i>Gerar Banner
                </button>
              </div>
            </div>
            <div id="bp-nota" class="border rounded p-3 bg-light" style="min-height:200px;font-size:0.85rem;white-space:pre-wrap;">
              <span class="text-muted fst-italic">Clique em "Gerar com IA" para gerar a nota técnica.</span>
            </div>
          </div>
        </div>
      </div>
...
<script>
// ══════════════════════════════════════════════════════════════════════════
// EXTRA PAUTA — Adicionar proposição manualmente
// ══════════════════════════════════════════════════════════════════════════
let _epDados = null;  // dados da proposição encontrada
let _epOrdemCounter = 900;  // ordens extras começam em 900 para não conflitar

function abrirModalAdicionarProposicao(){
  _epDados = null;
  document.getElementById('ep-resultado').style.display = 'none';
  document.getElementById('ep-erro').style.display = 'none';
  document.getElementById('ep-btn-adicionar').style.display = 'none';
  document.getElementById('ep-numero').value = '';
  document.getElementById('ep-ano').value = new Date().getFullYear();
  new bootstrap.Modal(document.getElementById('modalAdicionarProposicao')).show();
}

async function buscarProposicaoExtra(){
  const sigla  = document.getElementById('ep-sigla').value;
  const numero = document.getElementById('ep-numero').value.trim();
  const ano    = document.getElementById('ep-ano').value.trim();
  const errEl  = document.getElementById('ep-erro');
  const resEl  = document.getElementById('ep-resultado');
  const btnAdd = document.getElementById('ep-btn-adicionar');
  ...
  if(!numero || !ano){ errEl.textContent = 'Preencha número e ano.'; errEl.style.display = 'block'; return; }

  const btn = document.getElementById('ep-btn-buscar');
  btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Buscando...';

  try{
    const r = await fetch(`https://dadosabertos.camara.leg.br/api/v2/proposicoes?siglaTipo=${encodeURIComponent(sigla)}&numero=${numero}&ano=${ano}&itens=1`, {headers:{'Accept':'application/json'}});
    if(!r.ok) throw new Error('Erro HTTP ' + r.status);
    const dados = (await r.json()).dados || [];
    if(!dados.length){ errEl.textContent = `${sigla} ${numero}/${ano} não encontrado na API da Câmara.`; errEl.style.display = 'block'; return; }

    const prop = dados[0];
    // Busca autores
    let autor = 'N/D';
    try{
      const rA = await fetch(`https://dadosabertos.camara.leg.br/api/v2/proposicoes/${prop.id}/autores`,{headers:{'Accept':'application/json'}});
      if(rA.ok){ const autores = (await rA.json()).dados||[]; autor = autores.slice(0,2).map(a=>a.nome).join(', ') + (autores.length>2?' e outros':''); }
    }catch(e){}

    _epDados = {
      id_principal: String(prop.id),
      projeto: `${prop.siglaTipo} ${prop.numero}/${prop.ano}`,
      projeto_original: `${prop.siglaTipo} ${prop.numero}/${prop.ano}`,
      ementa: prop.ementa || '',
      autor,
      relator: 'A definir',
      orientacao: '',
      situacao: prop.statusProposicao?.descricaoSituacao || 'N/D',
      eh_extra_pauta: true,
    };

    document.getElementById('ep-preview').innerHTML =
      `<strong>${_epDados.projeto}</strong> — ${_epDados.ementa.substring(0,120)}${_epDados.ementa.length>120?'...':''}`;
    ...
    btn.disabled = false; btn.innerHTML = '<i class="fas fa-search me-1"></i>Buscar';
  }
}

async function adicionarProposicaoExtra(){
  if(!_epDados) return;

  _epOrdemCounter++;
  const ordem = _epOrdemCounter;
  const id    = _epDados.id_principal;
  const item  = {..._epDados, ordem: String(ordem)};

  // Adiciona ao ITENS_DATA global para Mensagens, Pauta Banner, etc.
  if(!window.ITENS_DATA) window.ITENS_DATA = [];
  window.ITENS_DATA.push({
    ordem: String(ordem), id_principal: id,
    projeto: item.projeto, projeto_original: item.projeto_original,
    ementa: item.ementa, autor: item.autor, relator: item.relator,
    orientacao: '', resumo_ia: '', responsavel_username: '',
    responsavel_nome: '', responsavel_categoria: '', eh_extra_pauta: true,
  });

  // Monta o card visual igual ao dos itens normais, mas com cor amarelo-escuro
  const container = document.querySelector('.container.my-4') || document.body;
  const card = document.createElement('div');
  card.className = 'item-card mb-3';
  card.dataset.idPrincipal = id;
  card.style.cssText = 'border:2px solid #B8860B;border-radius:8px;background:#FFFBEA;';
  card.id = `extra-pauta-${id}`;

  card.innerHTML = `
    <div class="card-body">
      <div class="item-header d-flex align-items-center justify-content-between"
           data-bs-toggle="collapse" data-bs-target="#col-ep-${id}" aria-expanded="false"
           style="cursor:pointer;color:#7B5E00;font-weight:600;">
        <div>
          <span class="badge me-2" style="background:#B8860B;font-size:0.7rem;">⭐ EXTRA PAUTA</span>
          <i class="fas fa-file-alt me-2" style="color:#B8860B;"></i>
          Item ${ordem} — <strong>${item.projeto}</strong>
          <button class="btn btn-sm btn-outline-danger ms-2 py-0 px-2" style="font-size:0.7rem;"
            onclick="event.stopPropagation();excluirExtraPauta('${id}',this.closest('.item-card'))">
            <i class="fas fa-trash-alt"></i> Excluir
          </button>
          <div class="item-info mt-1 text-muted fst-italic" style="font-size:0.82rem;">${item.ementa}</div>
          <div class="item-info mt-1 small"><strong>Autor:</strong> ${item.autor}</div>
        </div>
        <i class="fas fa-chevron-down collapse-toggle-icon ms-2"></i>
      </div>

      <div class="collapse" id="col-ep-${id}">
        <hr class="mt-2 mb-2">

        <div class="mb-2">
          <label class="form-label small"><strong>Gerar sugestão de análise:</strong></label>
          <div class="mb-2 d-flex align-items-center gap-2 flex-wrap">
            <button class="btn btn-outline-primary btn-gerar-analise btn-sm"
              data-ordem="${ordem}" data-projeto="${item.projeto.replace(/"/g,'')}"
              data-ementa="${item.ementa.replace(/"/g,'').replace(/'/g,'')}"
              data-autor="${item.autor.replace(/"/g,'')}" data-relator="${item.relator}"
              data-id-principal="${id}"
              onclick="gerarAnalise('${ordem}', this)">
              <img src="/static/logo_gemini.svg" style="width:16px;height:16px;vertical-align:middle;" class="me-1">Gerar Análise
            </button>
            <button class="btn btn-outline-secondary btn-gerar-banner btn-sm"
              data-ordem="${ordem}" data-proposicao="${item.projeto.replace(/"/g,'')}"
              data-ementa="${item.ementa.replace(/"/g,'').replace(/'/g,'')}"
              data-autor="${item.autor.replace(/"/g,'')}" data-relator="${item.relator}"
              data-id-principal="${id}"
              onclick="abrirModalBanner(this)">
        ...
        <div class="mt-2 position-relative">
          <label class="form-label small"><strong>Resumo/Nota Técnica:</strong></label>
          <textarea id="editor-resumo-materia-${ordem}" class="d-none"></textarea>
          <div id="quill-editor-${ordem}" data-ordem="${ordem}" style="width:100%;background:#fff;min-height:300px;"></div>
          <div id="overlay-${ordem}" class="editor-overlay" style="display:none;">
            <div class="editor-spinner"></div>
        ...
        <div class="mt-2">
          <label class="form-label small"><strong>Orientação:</strong></label>
          <select class="form-control editable-field orientacao" data-ordem="${ordem}">
            <option value="">Selecione</option>
            <option value="SIM">SIM</option>
            <option value="NÃO">NÃO</option>
            <option value="LIBERADO">LIBERADO</option>
        ...
        <div class="mt-4">
          <div class="d-flex align-items-center gap-2 mb-2">
            <h6 class="mb-0"><i class="fas fa-thumbtack text-warning me-2"></i>Destaques</h6>
            <button class="btn btn-outline-warning btn-sm btn-buscar-destaques"
              data-id-proposicao="${id}" data-ordem="${ordem}" data-projeto="${item.projeto}">
              <i class="fas fa-search me-1"></i>Buscar Destaques
            </button>
            <button class="btn btn-outline-primary btn-sm" onclick="window.gerarTabelaDestaques('${item.projeto}')">
              <i class="fas fa-table me-1"></i>Tabela de Destaques
            </button>
          </div>
          <div id="tabela-destaques-${ordem}" style="display:none;">
            <div class="table-responsive">
              <table class="table table-sm table-striped align-middle">
                <thead class="table-warning"><tr>
                  <th>Número</th><th>Autoria</th><th>Descrição</th><th>Tipo</th><th>Situação</th><th>IA</th>
                </tr></thead>
                <tbody id="tbody-destaques-${ordem}"></tbody>
              </table>
            </div>
            <div id="sem-destaques-${ordem}" class="alert alert-info py-2" style="display:none;">Nenhum destaque encontrado.</div>
          </div>
        </div>

        <div class="d-flex align-items-center gap-3 mt-3">
          <button class="btn btn-warning btn-sm save-btn" data-ordem="${ordem}" data-id-principal="${id}"
            style="color:#1a1a1a;">Salvar</button>
          <small class="text-muted" id="info-salvo-${ordem}"></small>
        </div>
      </div>
    </div>`;

  // Encontra o último item da pauta e insere depois
  const items = document.querySelectorAll('.item-card');
  const lastItem = items[items.length - 1];
  if(lastItem) lastItem.after(card);
  else container.appendChild(card);

  // Inicializa Quill para o novo item quando o collapse abrir
  const collapseEl = card.querySelector(`#col-ep-${id}`);
  collapseEl.addEventListener('shown.bs.collapse', () => {
    if(window.quillInstances && !window.quillInstances[ordem]){
      const q = new Quill(`#quill-editor-${ordem}`, {
        theme:'snow',
        modules:{toolbar:[['bold','italic','underline'],[{'list':'ordered'},{'list':'bullet'}],['clean']]}
      });
      const TEMPLATE = `<p><strong>📘 Resumo Técnico</strong></p><p><br></p><p><strong>🟢 Pontos Positivos</strong></p><p><br></p><p><strong>🔴 Pontos Negativos</strong></p><p><br></p><p><strong>⚖️ Riscos Políticos e de Imagem</strong></p><p><br></p><p><strong>↔️ Orientação Sugerida</strong></p><p><br></p>`;
      q.clipboard.dangerouslyPasteHTML(TEMPLATE);
      window.quillInstances[ordem] = q;
    }
  });

  // Salva no banco para persistência
  try{
    await fetch('/extra_pauta/' + window.EVENTO_ID, {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        id_principal: id, projeto: item.projeto,
        ementa: item.ementa, autor: item.autor, relator: item.relator,
      })
    });
  }catch(e){ console.warn('Erro ao persistir extra pauta:', e); }

  // Fecha o modal e abre o card
  bootstrap.Modal.getInstance(document.getElementById('modalAdicionarProposicao'))?.hide();
  setTimeout(() => { new bootstrap.Collapse(collapseEl, {toggle:true}); }, 400);
}


// ══════════════════════════════════════════════════════════════════════════
// BUSCAR PROPOSIÇÃO — Nota técnica, PDF, Banner
// ══════════════════════════════════════════════════════════════════════════
let _bpDados = null;

function abrirModalBuscarProposicao(){
  _bpDados = null;
  document.getElementById('bp-resultado').style.display = 'none';
  document.getElementById('bp-loading').style.display = 'none';
  document.getElementById('bp-numero').value = '';
  document.getElementById('bp-ano').value = new Date().getFullYear();
  document.getElementById('bp-nota').innerHTML = '<span class="text-muted fst-italic">Clique em "Gerar com IA" para gerar a nota técnica.</span>';
  new bootstrap.Modal(document.getElementById('modalBuscarProposicao')).show();
}

async function buscarProposicaoBP(){
  const sigla  = document.getElementById('bp-sigla').value;
  const numero = document.getElementById('bp-numero').value.trim();
  const ano    = document.getElementById('bp-ano').value.trim();
  if(!numero || !ano){ alert('Preencha número e ano.'); return; }
  ...
  try{
    // Usa backend para evitar CORS e ter fallbacks de busca
    const r = await fetch(`/buscar_proposicao?sigla=${encodeURIComponent(sigla)}&numero=${encodeURIComponent(numero)}&ano=${encodeURIComponent(ano)}`);
    if(!r.ok) throw new Error('HTTP ' + r.status);
    const prop = await r.json();
    if(!prop.found) throw new Error(prop.error || `${sigla} ${numero}/${ano} não encontrado.`);

    _bpDados = {
      id: prop.id, projeto: prop.projeto,
      numero: prop.numero, ano: prop.ano, sigla: prop.sigla,
      ementa: prop.ementa||'', autor: prop.autor, relator: prop.relator,
      situacao: prop.situacao||'N/D',
    };

    document.getElementById('bp-titulo').textContent = _bpDados.projeto;
    document.getElementById('bp-ementa').textContent = _bpDados.ementa;
    document.getElementById('bp-autor').textContent   = _bpDados.autor;
    document.getElementById('bp-relator').textContent = _bpDados.relator;
    document.getElementById('bp-situacao').textContent= _bpDados.situacao;
    document.getElementById('bp-nota').innerHTML = '<span class="text-muted fst-italic">Clique em "Gerar com IA" para gerar a nota técnica.</span>';
    document.getElementById('bp-resultado').style.display = 'block';

    // Busca nota técnica no banco (inclui REQs de urgência relacionados)
    buscarNotaBancoBP(_bpDados.id);
...
async function buscarNotaBancoBP(idProposicao){
  const nota = document.getElementById('bp-nota');
  nota.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Buscando nota técnica no banco...';
  try{
    // Passa nome e número/ano para ajudar a encontrar REQs de urgência
    const bpProjeto  = (_bpDados && _bpDados.projeto)  ? encodeURIComponent(_bpDados.projeto)  : '';
    const bpNumero   = (_bpDados && _bpDados.numero)   ? encodeURIComponent(_bpDados.numero)   : '';
    const bpAno      = (_bpDados && _bpDados.ano)      ? encodeURIComponent(_bpDados.ano)      : '';
    const bpUrl = '/notas_por_proposicao/' + idProposicao
      + '?projeto=' + bpProjeto + '&numero=' + bpNumero + '&ano=' + bpAno;
    const r = await fetch(bpUrl);
    const j = await r.json();
    if(j.found && j.notas && j.notas.length > 0){
      const n = j.notas[0];
      const dataStr = n.saved_at ? ' · Salvo em ' + n.saved_at.substring(0,10).split('-').reverse().join('/') + ' por ' + (n.saved_by||'') : '';
      const tipoStr = (n.tipo === 'nota_req' || n.tipo === 'nota_req_pauta')
        ? ' <span class="badge bg-warning text-dark">Via REQ de Urgência</span>' : '';
      nota.style.whiteSpace = 'normal';
      nota.innerHTML =
        '<div class="alert alert-success py-1 px-2 mb-2 small">' +
        '<i class="fas fa-check-circle me-1"></i><strong>Nota encontrada no banco</strong>' + tipoStr + dataStr +
        '</div>' + n.resumo;
      _bpDados.nota = n.resumo;
      // Se há mais de uma nota, mostra aviso
      if(j.notas.length > 1){
        nota.innerHTML += '<div class="text-muted small mt-2">(' + j.notas.length + ' notas encontradas — exibindo a mais recente)</div>';
      }
    } else {
      nota.innerHTML = '<span class="text-muted fst-italic">Nenhuma nota técnica no banco. Clique em "Gerar com IA" para gerar.</span>';
    }
  }catch(e){
    nota.innerHTML = '<span class="text-muted fst-italic">Clique em "Gerar com IA" para gerar a nota técnica.</span>';
    console.warn('buscarNotaBancoBP:', e);
  }
}

async function gerarNotaBP(){
  if(!_bpDados){ alert('Busque uma proposição primeiro.'); return; }
  const nota = document.getElementById('bp-nota');
  nota.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Gerando análise com IA...';
  try{
    const r = await fetch('/analisar_ia', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({
        projeto: _bpDados.projeto, ementa: _bpDados.ementa,
        autor: _bpDados.autor, relator: _bpDados.relator,
        id_principal: _bpDados.id,
      })
    });
    const j = await r.json();
    if(j.resumo){
      nota.innerHTML = j.resumo;
      nota.style.whiteSpace = 'normal';
      _bpDados.nota = j.resumo;
    } else {
      nota.innerHTML = '<span class="text-danger">Erro: ' + (j.error||'Falha na IA') + '</span>';
    }
  }catch(e){
    nota.innerHTML = '<span class="text-danger">Erro: ' + e.message + '</span>';
  }
}

function imprimirNotaBP(){
  if(!_bpDados){ alert('Busque uma proposição primeiro.'); return; }
  const nota = document.getElementById('bp-nota').innerHTML;
  const popup = window.open('', '_blank', 'width=800,height=700,scrollbars=yes');
  const pd = popup.document;
  pd.open(); pd.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Nota Técnica</title><style>body{font-family:Arial,sans-serif;padding:30px;color:#222;line-height:1.6;}h2{color:#1A3A5C;border-bottom:2px solid #1A3A5C;padding-bottom:8px;}@media print{.no-print{display:none}}</style></head><body></body></html>'); pd.close();
  const h2 = pd.createElement('h2'); h2.textContent = _bpDados.projeto; pd.body.appendChild(h2);
  const sub = pd.createElement('p'); sub.style.color='#555'; sub.style.fontSize='13px'; sub.textContent = 'Ementa: ' + _bpDados.ementa; pd.body.appendChild(sub);
  const hr = pd.createElement('hr'); pd.body.appendChild(hr);
  const div = pd.createElement('div'); div.innerHTML = nota; pd.body.appendChild(div);
  const btnP = pd.createElement('button'); btnP.className='no-print'; btnP.style.cssText='margin-top:20px;padding:8px 20px;background:#1A3A5C;color:#fff;border:none;border-radius:4px;cursor:pointer;'; btnP.textContent='🖨️ Imprimir'; btnP.onclick=()=>popup.print(); pd.body.appendChild(btnP);
}

function excluirExtraPauta(idProp, cardEl){
  if(!confirm('Excluir esta proposição extra pauta?')) return;
  fetch('/extra_pauta/' + window.EVENTO_ID + '/' + idProp, {method:'DELETE'})
    .then(r => r.json())
    .then(j => {
      if(j.ok){
        cardEl?.remove();
        // Remove do ITENS_DATA
        if(window.ITENS_DATA) window.ITENS_DATA = window.ITENS_DATA.filter(it => String(it.id_principal) !== String(idProp) || !it.eh_extra_pauta);
      } else { alert('Erro ao excluir: ' + (j.error||'')); }
    })
    .catch(e => alert('Erro: ' + e.message));
}

async function carregarExtraPauta(){
  if(!window.EVENTO_ID) return;
  try{
    const r = await fetch('/extra_pauta/' + window.EVENTO_ID);
    if(!r.ok) return;
    const itens = await r.json();
    for(const item of itens){
      // Não duplica se já existe na tela
      if(document.getElementById('extra-pauta-' + item.id_principal)) continue;
      await adicionarExtraPautaCarregado(item);
    }
  }catch(e){ console.warn('Erro ao carregar extra pauta:', e); }
}

// Versão silenciosa de adicionar (sem modal, sem salvar no banco novamente)
async function adicionarExtraPautaCarregado(item){
  _epOrdemCounter++;
  const ordem = _epOrdemCounter;
  const id    = item.id_principal;
  item.ordem  = String(ordem);

  if(!window.ITENS_DATA) window.ITENS_DATA = [];
  if(!window.ITENS_DATA.find(it => String(it.id_principal)===String(id) && it.eh_extra_pauta)){
    window.ITENS_DATA.push({
      ordem: String(ordem), id_principal: id,
      projeto: item.projeto, projeto_original: item.projeto,
      ementa: item.ementa, autor: item.autor, relator: item.relator,
      orientacao: '', resumo_ia: '', responsavel_username: '',
      responsavel_nome: '', responsavel_categoria: '', eh_extra_pauta: true,
    });
  ...
  const dataStr = item.created_at ? ' · Adicionado em ' + item.created_at.substring(0,10).split('-').reverse().join('/') + ' por ' + (item.created_by||'') : '';
    ...
    <div class="card-body">
      <div class="item-header d-flex align-items-center justify-content-between"
        ...
        <div>
          ...
          <div class="item-info mt-1 text-muted fst-italic" style="font-size:0.82rem;">${item.ementa}</div>
          <div class="item-info mt-1 small"><strong>Autor:</strong> ${item.autor}${dataStr}</div>
        </div>
        <i class="fas fa-chevron-down collapse-toggle-icon ms-2"></i>
      </div>
      <div class="collapse" id="col-ep-${id}">
        ...
        <div class="mb-2">
          ...
          <div class="mb-2 d-flex align-items-center gap-2 flex-wrap">
            <button class="btn btn-outline-primary btn-gerar-analise btn-sm"
              data-ordem="${ordem}" data-projeto="${item.projeto.replace(/"/g,'')}"
              data-ementa="${item.ementa.replace(/"/g,'').replace(/'/g,'')}"
              data-autor="${item.autor.replace(/"/g,'')}" data-relator="${item.relator||'N/D'}"
              data-id-principal="${id}" onclick="gerarAnalise('${ordem}', this)">
              <img src="/static/logo_gemini.svg" style="width:16px;height:16px;vertical-align:middle;" class="me-1">Gerar Análise
            </button>
            <button class="btn btn-outline-secondary btn-gerar-banner btn-sm"
              data-ordem="${ordem}" data-proposicao="${item.projeto.replace(/"/g,'')}"
              data-ementa="${item.ementa.replace(/"/g,'').replace(/'/g,'')}"
              data-autor="${item.autor.replace(/"/g,'')}" data-relator="${item.relator||'N/D'}"
              data-id-principal="${id}" onclick="abrirModalBanner(this)">
              <i class="fas fa-image me-2"></i>Gerar Banner
            </button>
          </div>
        </div>
        <div class="mt-2 position-relative">
          ...
          <div id="quill-editor-${ordem}" data-ordem="${ordem}" style="width:100%;background:#fff;min-height:300px;"></div>
          <div id="overlay-${ordem}" class="editor-overlay" style="display:none;">
            <div class="editor-spinner"></div><p>Analisando proposição...</p>
          </div>
        </div>
        <div class="mt-2">
          <label class="form-label small"><strong>Orientação:</strong></label>
          <select class="form-control editable-field orientacao" data-ordem="${ordem}">
            ...
            <option value="LIBERADO">LIBERADO</option><option value="OBSTRUÇÃO">OBSTRUÇÃO</option>
            <option value="ABSTENÇÃO">ABSTENÇÃO</option><option value="NEGOCIAÇÃO">NEGOCIAÇÃO</option>
          </select>
        </div>
        <div class="mt-4">
          <div class="d-flex align-items-center gap-2 mb-2">
  ...
  const items = document.querySelectorAll('.item-card');
  const lastItem = items[items.length - 1];
  if(lastItem && lastItem !== card) lastItem.after(card);
  else container.appendChild(card);

  card.querySelector(`#col-ep-${id}`)?.addEventListener('shown.bs.collapse', () => {
    if(window.quillInstances && !window.quillInstances[ordem]){
      const q = new Quill(`#quill-editor-${ordem}`, {
        theme:'snow', modules:{toolbar:[['bold','italic','underline'],[{'list':'ordered'},{'list':'bullet'}],['clean']]}
      });
      const TEMPLATE = `<p><strong>📘 Resumo Técnico</strong></p><p><br></p><p><strong>🟢 Pontos Positivos</strong></p><p><br></p><p><strong>🔴 Pontos Negativos</strong></p><p><br></p><p><strong>⚖️ Riscos Políticos e de Imagem</strong></p><p><br></p><p><strong>↔️ Orientação Sugerida</strong></p><p><br></p>`;
      q.clipboard.dangerouslyPasteHTML(TEMPLATE);
      window.quillInstances[ordem] = q;
...
// Chama ao carregar a página (após 1.5s para garantir que DOM esteja pronto)
setTimeout(carregarExtraPauta, 1500);

function gerarBannerBP(){
  if(!_bpDados){ alert('Busque uma proposição primeiro.'); return; }
  // Popula o modal de gerar banner com os dados da proposição buscada
  // e abre normalmente
  const btnFake = {
    dataset: {
      proposicao: _bpDados.projeto, ementa: _bpDados.ementa,
      autor: _bpDados.autor, relator: _bpDados.relator,
      idPrincipal: _bpDados.id, ordem: '999',
    }
  };
  // Fecha modal buscar e abre modal de banner
  bootstrap.Modal.getInstance(document.getElementById('modalBuscarProposicao'))?.hide();
  setTimeout(() => abrirModalBanner(btnFake), 400);
}
</script>
</body>
</html>
